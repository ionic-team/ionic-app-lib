...
Browser.downloadCordovaCrosswalkEngine = function downloadCordovaCrosswalkEngine(appDirectory) {
  ...
  if (fs.existsSync(path.join(appDirectory, 'engine', cordovaCrosswalkFolderName))) {
    l.logger.debug('The Engine already exists for version ', crosswalkEngineVersion);
    q.resolve();
    return q.promise;
  }
  ...
  .then(function(data) {
    ...
    l.logger.error('Failed to download cordova-crosswalk-engine - %s', error, {});
    q.reject();
  })
...
Browser.copyInCrosswalkLibrariesToCrosswalkEngine = function copyInCrosswalkLibrariesToCrosswalkEngine(appDirectory, architecture, version) {
  ...
  l.logger.info(copyCompleteMessage);
};
...
Browser.downloadCrosswalkWebview = function downloadCrosswalkWebview(appDirectory, architecture, version, releaseStatus, isLiteVersion) {
  ...
  .then(function(data) {
    ...
    l.logger.error('Crosswalk webviews failed to download - %s', error, {});
    q.reject(error);
  });
...
Browser.downloadCrosswalkWebviews = function downloadCrosswalkWebviews(appDirectory, version, isLiteVersion) {
  ...
  l.logger.debug('Attempting to install crosswalk version: ', version);
  ...
  // l.logger.debug('xwalk', xwalkVersion)
...
Browser.downloadCordova40x = function downloadCordova40x(appDirectory) {
  ...
  .then(function(data) {
    ...
    l.logger.error('Failed to download cordova-android v4.0.x - %s', error, {});
    q.reject();
  })
...
Browser.downloadFiles = function downloadFiles(appDirectory, version, isLiteVersion) {
  ...
  .catch(function(error){
    l.logger.error('There was an error finishing the browser command - %s', error, {});
    // q.reject(error);
    throw error;
  });
...
Browser.installCordovaCrosswalk = function installCordovaCrosswalk(appDirectory) {
  ...
  l.logger.info('The ionic browser command will install Cordova Android, the Crosswalk plugin, the whitelist plugin, and the splashscreen plugin for your convenience.'.yellow);
  l.logger.info('Due to this, if you need to specify a version of Crosswalk, see this link: https://github.com/crosswalk-project/cordova-plugin-crosswalk-webview#configure'.yellow);
  l.logger.info('Enjoy! :)'.green);
  shelljs.exec('cordova platform add android');
  l.logger.info('Added Android'.green);
  shelljs.exec('cordova plugin add cordova-plugin-crosswalk-webview');
  l.logger.info('Added Crosswalk plugin'.green);
  shelljs.exec('cordova plugin add cordova-plugin-whitelist');
  l.logger.info('Added whitelist plugin'.green);
  shelljs.exec('cordova plugin add cordova-plugin-splashscreen');
  l.logger.info('Added splash screen plugin'.green);
  // return
  var completeMessage = 'Completed installing Crosswalk'.green.bold;
  l.logger.info(completeMessage);
...
Browser.installCrosswalk = function installCrosswalk(appDirectory, version, saveToPackageJson, isLiteVersion) {
  ...
  if (!info.cordova) {
    ...
    l.logger.info(cordovaMessage);
    return Q(cordovaMessage);
  } else if (semver.satisfies(info.cordova, '>=5.0.0')) {
    return Browser.installCordovaCrosswalk(appDirectory);
  } 

  l.logger.info(('You are running Cordova CLI ' + info.cordova + ' - installing Cordova Android and Crosswalk manually').yellow.bold);
...
Browser.clean = function clean(appDirectory) {
  l.logger.info('Cleaning up Ionic browser artifacts'.green);
  shelljs.rm('-rf', path.join(appDirectory, './engine'));
  l.logger.info('Cleaned'.yellow);
};

Browser.upgradeCrosswalk = function upgradeCrosswalk(appDirectory) {
  l.logger.info('Updating your Ionic project with the latest build of Crosswalk'.green);
...
Browser.revertBrowser = function revertBrowser(appDirectory, platform) {
  if (!platform) {
    l.logger.info('You did not specify a platform to revert the browser');
    return
  }

  if(platforms.indexOf(platform) == -1) {
    l.logger.info('You did not specify a valid platform.');
    return
  }

  l.logger.info('Reverting', platform, 'browser');
  ...
  l.logger.info('Reverted', platform, 'browser');
};
...
Browser.addBrowser = function addBrowser(appDirectory, browserToInstall, saveToPackageJson) {
  ...
  switch (browserToInstall) {
    case 'crosswalk':
      l.logger.info('Adding', browserToInstall, 'browser');
    ...
    //   break;
    default:
      l.logger.info('No accepted browser was specified.'.red.bold);
      validBrowserSpecified = false;
      promise = Q();
      break;
  }
  ...
  .catch(function(error) {
    l.logger.error('An error occured with addBrowser - %s', error, {});
    throw error;
  });
};
...
Browser.removeBrowser = function removeBrowser(appDirectory, browser) {
  ...
  if (!browser) {
    l.logger.warn('Please specify a browser to be removed');
    return;
  }

  l.logger.info('Removing', browser, 'browser');
    ...
    if (browser.available) {
      ...
      if (browser.versions) {
        for (version in browser.versions) {
          ...
          // l.logger.info('betacanary', betaCanaryMessage)
          // l.logger.info('beta canary msg', betaCanaryMessage)
          l.logger.info(betaCanaryMessage, 'Version'.blue, ver.version.yellow, 'Published'.cyan, new Date(ver.publish_date));
        }
      }
    } else {
      l.logger.info('Not Available Yet'.red.bold, '-', browser.name.green);
    }
  }
};

Browser.listBrowsers = function listBrowsers(appDirectory) {
  l.logger.info('iOS - Browsers Listing:\n');
...
Browser.listInstalledBrowsers = function listInstalledBrowsers(appDirectory) {
  ...
  for(browserIndex in browsers) {
    ...
    l.logger.info('For', browser.platform, '-', browser.browser, browser.version);
  }
