...
Browser.logger = logging.defaultLogger;
...
Browser.downloadCordovaCrosswalkEngine = function downloadCordovaCrosswalkEngine(appDirectory) {
  ...
  if (fs.existsSync(path.join(appDirectory, 'engine', cordovaCrosswalkFolderName))) {
    Browser.logger.debug('The Engine already exists for version ', crosswalkEngineVersion);
    q.resolve();
    return q.promise;
  }
  ...
  .then(function(data) {
    ...
    Browser.logger.error('Failed to download cordova-crosswalk-engine - %s', error, {});
    q.reject();
  })
...
Browser.copyInCrosswalkLibrariesToCrosswalkEngine = function copyInCrosswalkLibrariesToCrosswalkEngine(appDirectory, architecture, version) {
  ...
  Browser.logger.info(copyMessage);
  ...
  Browser.logger.info(copyCompleteMessage);
};
...
Browser.downloadCrosswalkWebview = function downloadCrosswalkWebview(appDirectory, architecture, version, releaseStatus, isLiteVersion) {
  ...
  .then(function(data) {
    ...
    Browser.logger.error('Crosswalk webviews failed to download - %s', error, {});
    q.reject(error);
  });
...
Browser.downloadCrosswalkWebviews = function downloadCrosswalkWebviews(appDirectory, version, isLiteVersion) {
  ...
  Browser.logger.debug('Attempting to install crosswalk version: ', version);
  ...
  // Browser.logger.debug('xwalk', xwalkVersion)
...
Browser.downloadCordova40x = function downloadCordova40x(appDirectory) {
  ...
  .then(function(data) {
    ...
    Browser.logger.error('Failed to download cordova-android v4.0.x - %s', error, {});
    q.reject();
  })
...
Browser.downloadFiles = function downloadFiles(appDirectory, version, isLiteVersion) {
  ...
  .catch(function(error){
    Browser.logger.error('There was an error finishing the browser command - %s', error, {});
    // q.reject(error);
    throw error;
  });
...
Browser.installCordovaCrosswalk = function installCordovaCrosswalk(appDirectory) {
  Browser.logger.info('*Info*\nCordova CLI v5.0 no longer requires you to use the ionic browser command to get Crosswalk installed'.yellow);
  Browser.logger.info('The ionic browser command will install Cordova Android, the Crosswalk plugin, the whitelist plugin, and the splashscreen plugin for your convenience.'.yellow);
  Browser.logger.info('Due to this, if you need to specify a version of Crosswalk, see this link: https://github.com/crosswalk-project/cordova-plugin-crosswalk-webview#configure'.yellow);
  Browser.logger.info('Enjoy! :)'.green);
  shelljs.exec('cordova platform add android');
  Browser.logger.info('Added Android'.green);
  shelljs.exec('cordova plugin add cordova-plugin-crosswalk-webview');
  Browser.logger.info('Added Crosswalk plugin'.green);
  shelljs.exec('cordova plugin add cordova-plugin-whitelist');
  Browser.logger.info('Added whitelist plugin'.green);
  shelljs.exec('cordova plugin add cordova-plugin-splashscreen');
  Browser.logger.info('Added splash screen plugin'.green);
  // return
  var completeMessage = 'Completed installing Crosswalk'.green.bold;
  Browser.logger.info(completeMessage);
...
Browser.installCrosswalk = function installCrosswalk(appDirectory, version, saveToPackageJson, isLiteVersion) {
  ...
  if (!info.cordova) {
    ...
    Browser.logger.info(cordovaMessage);
    return Q(cordovaMessage);
  } else if (semver.satisfies(info.cordova, '>=5.0.0')) {
    return Browser.installCordovaCrosswalk(appDirectory);
  } 

  Browser.logger.info(('You are running Cordova CLI ' + info.cordova + ' - installing Cordova Android and Crosswalk manually').yellow.bold);
...
Browser.clean = function clean(appDirectory) {
  Browser.logger.info('Cleaning up Ionic browser artifacts'.green);
  shelljs.rm('-rf', path.join(appDirectory, './engine'));
  Browser.logger.info('Cleaned'.yellow);
};

Browser.upgradeCrosswalk = function upgradeCrosswalk(appDirectory) {
  Browser.logger.info('Updating your Ionic project with the latest build of Crosswalk'.green);
...
Browser.revertBrowser = function revertBrowser(appDirectory, platform) {
  if (!platform) {
    Browser.logger.info('You did not specify a platform to revert the browser');
    return
  }

  if(platforms.indexOf(platform) == -1) {
    Browser.logger.info('You did not specify a valid platform.');
    return
  }

  Browser.logger.info('Reverting', platform, 'browser');
  ...
  Browser.logger.info('Reverted', platform, 'browser');
};
...
Browser.addBrowser = function addBrowser(appDirectory, browserToInstall, saveToPackageJson) {
  ...
  switch (browserToInstall) {
    case 'crosswalk':
      Browser.logger.info('Adding', browserToInstall, 'browser');
    ...
    //   break;
    default:
      Browser.logger.info('No accepted browser was specified.'.red.bold);
      validBrowserSpecified = false;
      promise = Q();
      break;
  }
  ...
  .catch(function(error) {
    Browser.logger.error('An error occured with addBrowser - %s', error, {});
    throw error;
  });
};
...
Browser.removeBrowser = function removeBrowser(appDirectory, browser) {
  ...
  if (!browser) {
    Browser.logger.warn('Please specify a browser to be removed');
    return;
  }

  Browser.logger.info('Removing', browser, 'browser');
    ...
    if (browser.available) {
      ...
      if (browser.versions) {
        for (version in browser.versions) {
          ...
          // Browser.logger.info('betacanary', betaCanaryMessage)
          // Browser.logger.info('beta canary msg', betaCanaryMessage)
          Browser.logger.info(betaCanaryMessage, 'Version'.blue, ver.version.yellow, 'Published'.cyan, new Date(ver.publish_date));
        }
      }
    } else {
      Browser.logger.info('Not Available Yet'.red.bold, '-', browser.name.green);
    }
  }
};

Browser.listBrowsers = function listBrowsers(appDirectory) {
  Browser.logger.info('iOS - Browsers Listing:\n');
...
Browser.listInstalledBrowsers = function listInstalledBrowsers(appDirectory) {
  ...
  for(browserIndex in browsers) {
    ...
    Browser.logger.info('For', browser.platform, '-', browser.browser, browser.version);
  }
