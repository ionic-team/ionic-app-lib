...
Browser.getCordovaCrosswalkEngine = function getCordovaCrosswalkEngine(appDirectory) {
  var q = Q.defer();
...
Browser.copyInCrosswalkLibrariesToCrosswalkEngine = function copyInCrosswalkLibrariesToCrosswalkEngine(architecture, version) {
  ...
  // events.emit('log', 'copying - ', copySource, copyDestination)
...
Browser.downloadCrosswalkWebview = function downloadCrosswalkWebview(architecture, version, releaseStatus) {
  var q = Q.defer();
  var self = this;
  ...
  .then(function(data) {
    ...
    events.emit('log', 'Crosswalk webviews failed to download - ');
    q.reject(error);
  });
...
Browser.getCrosswalkWebviews = function getCrosswalkWebviews(appDirectory, version) {
...
Browser.addCrosswalkPlugin = function addCrosswalkPlugin(appDirectory) {
  ...
  // events.emit('log', 'add crosswalk plugin', command);
  // var result = shelljs.exec(command);
  // events.emit('log', '\nAdded Crosswalk Webview Engine');
};

Browser.removeCrosswalkEngines = function removeCrosswalkEngines(appDirectory) {
  return Cordova.removePlugin(appDirectory, 'org.apache.cordova.engine.crosswalk')
  .then(function(){
    return Cordova.removePlugin(appDirectory, 'org.crosswalk.engine');
  })
  .then(function() {
    return Cordova.removePlugin(appDirectory, 'cordova-plugin-crosswalk-webview');
  })
  .catch(function(error) {
    // throw error;
  });
...
Browser.downloadFiles = function downloadFiles(appDirectory, version) {
  ...
  return Browser.getCordovaCrosswalkEngine(appDirectory)
  .then(function(data) {
  ...
  .then(function(data) {
    ...
    return Browser.getCrosswalkWebviews(appDirectory, version);
  })
  // .then(function(data) {
...
Browser.addBrowser = function addBrowser(appDirectory, browserToInstall) {
  ...
  switch (browserToInstall) {
    case 'crosswalk':
      ...
      Browser.installCrosswalk(appDirectory, browserVersion);
