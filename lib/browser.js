...
Browser.downloadCrosswalkWebview = function downloadCrosswalkWebview(appDirectory, architecture, version, releaseStatus) {
  var q = Q.defer();
...
Browser.downloadCrosswalkWebviews = function downloadCrosswalkWebviews(appDirectory, version) {
  // var version = version ? version : '8.37.189.14';

  events.emit('verbose', 'Attempting to install crosswalk version: ', version);

  var xwalkVersion = _.findWhere(Browser.crosswalkVersions, { version: version });
  ...
  var armPromise = Browser.downloadCrosswalkWebview(appDirectory, 'arm', version, releaseStatus);
  var x86Promise = Browser.downloadCrosswalkWebview(appDirectory, 'x86', version, releaseStatus);
...
Browser.setPermissionsForFolder = function setPermissionsForFolder(folderName) {
  console.log('$$$$$$$Setting permissions for foldername', folderName);
...
Browser.removeAndroidProject = function removeAndroidProject(appDirectory) {
  return Cordova.removePlatform(appDirectory, 'android')
  .then(function() {
    events.emit('log', '\nRemoved old Cordova Android platform');
  })
};

Browser.addCordova40xProject = function addCordova40xProject(appDirectory) {
  var cordovaAndroidPath = ['./engine/cordova-android-', cordovaAndroidVersion, '/'].join('');
  // var cordovaAndroidPath = 'android@4.0.0';
  return Cordova.addPlatform(appDirectory, cordovaAndroidPath)
  .then(function() {
...
Browser.addWhitelistPlugin = function addWhitelistPlugin(appDirectory) {
  var whitelistPlugin = 'https://github.com/apache/cordova-plugin-whitelist.git#r1.0.0';
  return Cordova.addPlugin(appDirectory, whitelistPlugin)
  .then(function() {
    events.emit('log', '\nAdded Crosswalk Webview Engine');
  });
};

Browser.addCrosswalkPlugin = function addCrosswalkPlugin(appDirectory) {
  ...
  return Cordova.addPlugin(appDirectory, crosswalkEnginePath)
  .then(function() {
    events.emit('log', '\nAdded Crosswalk Webview Engine');
  })
  .catch(function(error) {
...
Browser.removeCrosswalkEngines = function removeCrosswalkEngines(appDirectory) {
  Cordova.removePlugin(appDirectory, 'org.apache.cordova.engine.crosswalk');
  Cordova.removePlugin(appDirectory, 'org.crosswalk.engine');
  Cordova.removePlugin(appDirectory, 'cordova-plugin-crosswalk-webview');
};

Browser.addSplashScreenPlugin = function addSplashScreenPlugin(appDirectory) {
  return Cordova.addPlugin(appDirectory, 'org.apache.cordova.splashscreen')
};
...
Browser.downloadFiles = function downloadFiles(appDirectory, version) {
  // var q = Q.defer();
  ...
  .then(function(data) {
    ...
    return Browser.downloadCrosswalkWebviews(appDirectory, version);
  })
  // .then(function(data) {
...
Browser.installCrosswalk = function installCrosswalk(appDirectory, version) {
  return Browser.downloadFiles(appDirectory, version)
  .then(function(data) {
    return Browser.removeAndroidProject(appDirectory);
  })
  .then(function(){
    return Browser.removeCrosswalkEngines(appDirectory);
  })
  .then(function() {
    return Browser.addCordova40xProject(appDirectory);
  })
  .then(function() {
    return Browser.addCrosswalkPlugin(appDirectory);
  })
  .then(function() {
    return Browser.addWhitelistPlugin(appDirectory);
  })
  .then(function() {
    Browser.addGradleProperties(appDirectory);
    return Browser.addSplashScreenPlugin(appDirectory);
...
Browser.addBrowser = function addBrowser(appDirectory, browserToInstall) {
  ...
  switch (browserToInstall) {
    case 'crosswalk':
      ...
      promise = Browser.installCrosswalk(appDirectory, browserVersion);
