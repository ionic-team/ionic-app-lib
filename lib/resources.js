...
function generate(dir, options) {
  ...
  if (options.default) {
    if (hasExistingResources() && !options.force) {
    ...
    return copyIconFilesIntoResources(options.force)
      .then(function() {
  ...
  if (_.contains(options.platforms, 'all')) {
    ...
    buildPlatforms = _.intersection(supportedPlatforms, listedPlatforms);
  ...
  return getConfigData().then(function() {
...
function queueResTypeImages(resType) {
  ...
  function buildImagesData() {
    ...
    buildPlatforms.forEach(function(platform) {
      ...
      var platformResourceDir = path.join(appDirectory, Settings.resourceDir, platform);
...
function getConfigData() {
  ...
  try {
    fs.readFile(path.join(appDirectory, Settings.configFile), onConfigRead);
  } catch (err) {
    logging.logger.error('Error saveConfigData: ' + err);
    deferred.reject();
  }
...
function copyIconFilesIntoResources(forceAddResources) {
  ...
  // logging.logger.info('uzip to: ', unzipPath)
