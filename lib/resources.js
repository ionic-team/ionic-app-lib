...
function generate(options, v) {
  ...
  if(options.default) {
    ...
    copyIconFilesIntoResources(options.force)
    .then(function() {
  ...
  if (_.contains(options._, 'ios')) {
    buildPlatforms.push('ios');

  } else if (_.contains(options._, 'android')) {
    buildPlatforms.push('android');

  } else if (_.contains(options._, 'wp8')) {
