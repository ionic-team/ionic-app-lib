...
Utils.isIonicV2 = function isIonicV2(appDirectory) {
  var info = {},
      semver = require('semver');
  Info.getIonicVersion(info, appDirectory);
  try {
    var ionicVersion = semver.parse(info.ionic);
    if (ionicVersion.major >= 2) {
      return true;
    }
  } catch (ex) { }
  return false;
...
Utils.findIonicRoot = function findIonicRoot(dir) {
  ...
  Ionic2Project = require('./v2/project');
  ...
  for (var i = 0; i < 1000; ++i) {
    ...
    if (fs.existsSync(path.join(dir, Ionic2Project.CONFIG_FILE))) {
      return dir;
    }
...
Utils.cdIonicRoot = function cdIonicRoot(isV2) {
  ...
  var projFile = isV2 ? Ionic2Project.CONFIG_FILE : IonicProject.PROJECT_FILE;
  var rootDir = this.findIonicRoot();
  if (!rootDir) {
    throw new Error('Couldn\'t find ' + projFile + ' file. Are you in an Ionic project?');
  }
