...
Utils.fetchArchive = function fetchArchive(targetPath, archiveUrl) {
  ...
  var unzipRepo = function unzipRepo(fileName) {
    ...
    readStream.on('error', function(err) {
      l.logger.debug(('unzipRepo readStream: ' + err).error);
      q.reject(err);
    });
    ...
    writeStream.on('error', function(err) {
      l.logger.debug(('unzipRepo writeStream: ' + err).error);
      q.reject(err);
    });
  ...
  request({ url: archiveUrl, rejectUnauthorized: false, encoding: null, proxy: proxy }, function(err, res, body) {
    ...
    try {
      ...
      l.logger.debug('fetchArchive request write: ', e);
      q.reject(e);
    }
...
Utils.preprocessCliOptions = function preprocessCliOptions(argv) {
  l.logger.debug('Utils.preprocessCliOptions', argv);
  try {
    var options = {};
    ...
    l.logger.debug('An error occrured processing the CLI arguments', ex);
...
Utils.getContentSrc = function getContentSrc(appDirectory) {
  l.logger.debug('Utils.getContentSrc', appDirectory);
  var contentSrc;
  try {
    var fs = require('fs');
    var path = require('path');
    ...
    l.logger.debug('Utils.getContentSrc failed', e);
    console.log(e.stack);
...
Utils.fail = function fail(msg, taskHelp) {
  try {
    l.logger.debug('Utils.fail', msg, taskHelp);
    l.logger.debug('Utils.fail stack', msg.stack);

    //If an error handler is set, use it. Otherwise, just print basic info.
    if (Utils.errorHandler) {
      l.logger.debug('Utils.errorHandler is set, calling that now');
      return Utils.errorHandler(msg, taskHelp);
    }

    l.logger.error('An error occurred in Ionic App Lib and no error handler was set.');
    l.logger.error(msg);
    process.exit(1);
    return '';
  } catch (ex) {
    l.logger.debug('Utils.fail: ', ex);
  }
}
