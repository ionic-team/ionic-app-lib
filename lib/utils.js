 ...
 * @return {Promise}
 */
function createArchive(appDirectory, documentRoot) {
  var q = Q.defer();
...
function fetchArchive(targetPath, archiveUrl, isGui) {
  var os = require('os');
  var fs = require('fs');
  var path = require('path');
...
function preprocessCliOptions(argv) {
  ...
  try {
    ...
    options.targetPath = getProjectDirectory(options);
    ...
    fail('There was an error parsing out options from the Command Line');
  }
}
...
function getContentSrc(appDirectory) {
  ...
  try {
    ...
    parseString(configString, function(err, jsonConfig) {
      if (err) {
        return fail('Error parsing config.xml: ' + err);
      }
      try {
        contentSrc = jsonConfig.widget.content[0].$.src;
      } catch (e) {
        return fail('Error parsing ' + configXmlPath + ': ' + e.stack);
      }
    });

  } catch (e) {
    return fail('Error loading ' + configXmlPath + ': ' + e.stack);
  }
...
function fail(msg, taskHelp) {
  try {
    log.debug('fail', msg, taskHelp);
    log.debug('Utils.fail stack', msg.stack);

    // If an error handler is set, use it. Otherwise, just print basic info.
    if (errorHandler) {
      log.debug('errorHandler is set, calling that now');
      return errorHandler(msg, taskHelp);
    }
...
function gulpInstalledGlobally() {
...
function findIonicRoot(dir) {
  ...
  if (!dir) {
    ...
    if (pwd && pwd !== cwd && pwd !== 'undefined') {
      return findIonicRoot(pwd) || findIonicRoot(cwd);
    }
    return findIonicRoot(cwd);
  }
...
module.exports = {
  ...
  prepreocessCliOptions: preprocessCliOptions,
  getProjectDirectory: getProjectDirectory,
  getContentSrc: getContentSrc,
  mergeOptions: mergeOptions,
  fail: fail,
  gulpInstalledGlobally: gulpInstalledGlobally,
  cordovaInstalled: cordovaInstalled,
