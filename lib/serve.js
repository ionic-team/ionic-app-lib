...
Serve.gulpInstalledGlobally = function gulpInstalledGlobally() {
...
Serve.openBrowser = function openBrowser(options) {
  if(options.launchLab || options.launchBrowser) {
    var hostAddress = options.address == '0.0.0.0' ? 'localhost' : options.address;

    var open = require('open');
    var openUrl = options.launchLab ? [Serve.host(hostAddress, options.port), IONIC_LAB_URL] : [Serve.host(hostAddress, options.port)];
...
Serve.gulpStartupTasks = function gulpStartupTasks(options) {
  ...
  if ((options.gulpStartupTasks && options.gulpStartupTasks.length) || options.watchSass) {
    ...
    if(!Serve.gulpInstalledGlobally()) {
