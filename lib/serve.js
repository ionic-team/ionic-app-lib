...
Serve.loadSettings = function loadSettings(argv, project) {
  ...
  if (!project) {
    errorMessage = 'Serve.loadSettings - You must pass a project object to pull out project specific settings from';
    logging.logger.info(errorMessage);
    throw new Error(errorMessage);
  }
  ...
  try {
    ...
    if (addresses.length > 0) {
      ...
      for (var n = 0; n < addresses.length; n++) {
        logging.logger.info((' ' + (n + 1) + ') ' + addresses[n].address + ( addresses[n].dev ? ' (' + addresses[n].dev + ')' : '' )).yellow);
      }
