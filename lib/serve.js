...
Serve.listenForServerCommands = function listenForServerCommands(options) {
  ...
  rl.on('line', function(entry) {
    ...
    if(input == 'restart' || input == 'r') {
      ...
      l.logger.info('Console log output: '.green + (options.printConsoleLogs ? 'enabled' : 'disabled'));
      ...
      l.logger.info('Server log output: '.green + (options.printServerLogs ? 'enabled' : 'disabled'));
    ...
    // l.logger.debug('input: ', input);
    rl.prompt();
  }).on('close', function() {
    if (options.childProcess) {
      l.logger.info('Closing Gulp process'.yellow);
...
Serve.checkPorts = function(isIonicServer, testPort, testHost, options) {
  // l.logger.debug('Serve.checkports isIonicServer', isIonicServer, 'testPort:', testPort, 'testHost', testHost, 'options:', options);
  var q = Q.defer();
  var message = [];
  ...
  ports.getPort({port: testPort, host: testHost},
    ...
    if(port != testPort) {
      ...
      l.logger.info(message.yellow.bold);
      if(isIonicServer) {
        options.port = port;
      } else {
...
Serve.printCommandTips = function(ionic) {
  l.logger.info('Ionic server commands, enter:'.green.bold);
  l.logger.info('  restart' + ' or '.green + 'r' + ' to restart the client app from the root'.green);
  l.logger.info('  goto' + ' or '.green + 'g' + ' and a url to have the app navigate to the given url'.green);
  l.logger.info('  consolelogs' + ' or '.green + 'c' + ' to enable/disable console log output'.green);
  l.logger.info('  serverlogs' + ' or '.green + 's' + ' to enable/disable server log output'.green);
  l.logger.info('  quit' + ' or '.green + 'q' + ' to shutdown the server and exit'.green);
  l.logger.info('');
};
...
Serve.openBrowser = function openBrowser(options) {
  if(options.launchLab || options.launchBrowser) {
    ...
    try {
      ...
      l.logger.info('Error opening the browser - ', ex);
    }
  }
}
...
Serve.checkForDocumentRoot = function checkForDocumentRoot(options) {
  if (!fs.existsSync( path.join(options.appDirectory, options.documentRoot) )) {
    if (options.createDocumentRoot) {
      ...
      l.logger.info('"' + options.documentRoot + '" directory cannot be found. Please make sure the working directory is an Ionic project.');
      return false;
...
Serve.gulpStartupTasks = function gulpStartupTasks(options) {
  ...
  if ((options.gulpStartupTasks && options.gulpStartupTasks.length) || options.watchSass) {
    ...
    l.logger.info('Gulp startup tasks:'.green.bold, tasks);
...
Serve.runLivereload = function runLivereload(options, app) {
  ...
  try {
    ...
    // l.logger.debug('Absolute watch paths:', absoluteWatchPaths);
    ...
    q.reject(ex);
    l.logger.error('Live Reload failed to start, error: %s', ex, {});
    throw ex;
...
Serve.setupProxy = function setupProxy(options, app) {
  if(options.useProxy) {
    ...
    for (var x=0; x < options.proxies.length; x++) {
      ...
      l.logger.info('Proxy added:'.green.bold, proxy.path + " => " + url.format(proxy.proxyUrl));
    }
  }
};
...
Serve.startServer = function startServer(options, app) {
  ...
  l.logger.debug('Starting server at host address - ' + options.devServer);
  ...
  l.logger.info('Running dev server: '.green.bold, options.devServer);
};
...
Serve.start = function start(options) {
  ...
  try {
    ...
    .then(function(server) {
      l.logger.info('Running live reload server:'.green.bold, options.liveReloadServer);
      l.logger.info('Watching:'.green.bold, options.watchPatterns);
      if (options.useProxy) {
    ...
    .catch(function(error) {
      l.logger.error('Ionic serve failed - error: %s', error, {});
    });
  } catch(e) {
    var msg;
...
Serve._changed = function(filePath, options) {
  ...
  if( filePath.indexOf('.css') > 0 ) {
    l.logger.info(('CSS changed:  ' + filePath).green);
  } else if( filePath.indexOf('.js') > 0 ) {
    l.logger.info(('JS changed:   ' + filePath).green);
  } else if( filePath.indexOf('.html') > 0 ) {
    l.logger.info(('HTML changed: ' + filePath).green);
  } else {
    l.logger.info(('File changed: ' + filePath).green);
  }
...
Serve._goToUrl = function(url, options) {
  l.logger.info(('Loading: ' + url).green);
...
Serve._goToHistory = function(goHistory, options) {
  ...
  l.logger.info(('History Go: ' + goHistory).green);
...
Serve._postToLiveReload = function(files, options) {
  ...
  request.post(postUrl, {
    ...
    if(err) {
      l.logger.error('Unable to update live reload: %s', err, {});
    }
  });
...
Serve.getAddress = function(options) {
  ...
  try {
    ...
    if (addresses.length > 0) {
      ...
      l.logger.info('Please select which address to use by entering its number from the list below:'.error.bold);
      if (options.isPlatformServe) {
        l.logger.info('Note that the emulator/device must be able to access the given IP address'.small);
      }

      for (var x=0; x < addresses.length; x++) {
        l.logger.info((' ' + (x+1) + ') ' + addresses[x].address + ( addresses[x].dev ? ' (' + addresses[x].dev + ')' : '' )).yellow);
      }
      ...
      prompt.get({properties: promptProperties}, function (err, promptResult) {
        if(err && err.message !== 'canceled') {
          l.logger.debug('User prompted to select address - an error occured: %s', err, {});
          q.reject(err);
          return l.logger.error(err);
        }
        // } else {
        ...
        for(var x=0; x<addresses.length; x++) {
          if(selection == (x + 1) || selection == addresses[x].address || selection == addresses[x].dev) {
            options.address = addresses[x].address;
            if(!options.isAddressCmd) {
              l.logger.info('Selected address: '.green.bold + options.address);
            }
...
Serve.stopServer = function stopServer() {
  if (runningServer) {
    runningServer.close();
    lrServer.close();
    l.logger.info('Server closed');
  } else {
    l.logger.info('Server not running');
  }
}
