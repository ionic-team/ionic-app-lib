...
Serve.serveV2 = function serveV2(options) {
  // logging.logger.info('Serving your Ionic V2 application'.green.bold);
  try {
    logging.logger.debug('Serve options:', options);

    var compilePromise,
        openBrowser = options.launchBrowser,
        browserToOpen = 'google chrome';

    //Figure out better way to set default. ionic.project?
    options.watchPatterns = ['www/build/**/*'];

    var browserSyncConfig = {
      server: {
        baseDir: options.documentRoot || 'www',
      },
      port: options.port || DEFAULT_HTTP_PORT,
      files: options.watchPatterns || [ 'www/**/*.html' ],
      notify: false
    };
    ...
    if (!openBrowser) { 
      ...
      browserSyncConfig.browser = browserToOpen;
    ...
    if (!options.nobuild) {
      ...
      sassCompile = Transpile.processSass(options.appDirectory);
    } else {
      sassCompile = Q();
    }
    ...
    .then(function() {
      if (!options.nobuild) {
        logging.logger.debug('∆ Using Webpack to pack your files.\n'.yellow.bold);
        return Transpile.compile(options.appDirectory, options.watch, function() { });
      } else {
        return Q(false);
      }
    })
    .then(function(val) {
      ...
      logging.logger.info('∆ Starting browser sync.\n'.yellow.bold);
      logging.logger.debug('Browser sync config:', browserSyncConfig);
      return browserSync(browserSyncConfig);
    })
    .then(function(browserSyncInstance) {
