var fs = require('fs'),
    ...
    Transpile = require('./v2/transpile');
...
Serve.loadSettings = function loadSettings(argv, project) {
  ...
  // v2 options
  options.fontsOutputPath = project.get('fontsOutputPath') || '/www/build/fonts';
  options.sassEntryFile = project.get('sassEntryFile') || '/www/app/app.scss';
  options.sassOutputFile = project.get('sassOutputFile') || '/www/build/css/app.css';
  options.sassIncludePaths = project.get('sassIncludePaths') || ['node_modules/ionic-framework/src/scss'];
  options.sassWatchPatterns = project.get('sassWatchPatterns') || ['www/**/*.scss'];
  options.htmlWatchPatterns = project.get('htmlWatchPatterns') || ['www/**/*.html'];
