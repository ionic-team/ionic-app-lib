...
Serve.serveV2 = function serveV2(options) {
  ...
  try {
    ...
    if (options.compile) {
      // logging.logger.info('Compiling your files.'.yellow.bold);
      compilePromise = Transpile.compile(options.appDirectory, options.watch, function() {
        logging.logger.debug('Compilation step complete');
      });
    } else {
      compilePromise = Q();
    }
    ...
    .then(function(val) {
      logging.logger.info('Starting browser sync:'.green.bold);
