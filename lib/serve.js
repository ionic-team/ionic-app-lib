...
function listenForServerCommands(options) {
  ...
  rl.on('line', function(entry) {
    ...
    if (input === 'restart' || input === 'r') {
      ...
      _goToUrl('/?restart=' + Math.floor((Math.random() * 899999) + 100000), options);
...
function checkPorts(isIonicServer, testPort, testHost, options) {
...
function loadSettings(argv, project) {
  var errorMessage;
...
function checkForDocumentRoot(options) {
...
function runLivereload(options, app) {
  var q = Q.defer();
...
function startServer(options, app) {
...
function showFinishedServeMessage(options) {
  Serve.printCommandTips(options);
  Serve.listenForServerCommands(options);
}

function serverLog(req, msg, options) {
...
function _changed(filePath, options) {
...
function _goToUrl(url, options) {
  log.info('Loading: ' + url);
  Serve._postToLiveReload(['__ionic_goto_url__' + url], options);
}


function _goToHistory(goHistory, options) {
...
function _postToLiveReload(files, options) {
...
module.exports = {
  listenForServerCommands: listenForServerCommands,
  checkPorts: checkPorts,
  loadSettings: loadSettings,
  printCommandTips: printCommandTips,
  openBrowser: openBrowser,
  checkForDocumentRoot: checkForDocumentRoot,
  runLivereload: runLivereload,
  setupProxy: setupProxy,
  startServer: startServer,
  start: start,
  showFinishedServeMessage: showFinishedServeMessage,
  serverLog: serverLog,
  consoleLog: consoleLog,
  _changed: _changed,
  _goToUrl: _goToUrl,
  _goToHistory: _goToHistory,
  _postToLiveReload: _postToLiveReload,
