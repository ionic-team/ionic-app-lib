...
Transpile.compile = function compile(appDirectory, watch, cb) {
  ...
  try {
    ...
    logging.logger.info(['√ Using your webpack.config.js file:', webPackConfigPath, '\n'].join(' ').blue.bold);
  } catch (ex) {
    //No config existed. Use default;
    logging.logger.info('√ Using default Ionic webpack.config settings.'.blue.bold);
...
Transpile.processSass = function processSass(options) {
  ...
  Transpile.checkBuildDirectories(options.appDirectory);

  Transpile.prepareFonts(options.appDirectory);
  ...
  logging.logger.info('∆ Compiling your SASS to CSS.\n'.yellow.bold);

  var sassFile = path.join(options.appDirectory, 'www', 'app', 'app.scss');
  var includePaths = [ path.join(options.appDirectory, 'node_modules', 'ionic-framework', 'src', 'scss') ];
  ...
  logging.logger.info('√ Sass compiled to CSS.\n'.green.bold);
  logging.logger.debug('Sass output:\n', sassOutput.css.toString());

  var prefixerOutput = autoprefixer(autoprefixerOpts);

  var buildFilePath = path.join(options.appDirectory, 'www', 'build', 'css', 'app.css');  

  logging.logger.info('∆ Running postcss and autoprefixer on compiled CSS.\n'.yellow.bold);

  var cssPromise = postcss([ prefixerOutput ]).process(sassOutput.css.toString())
  .then(function (result) {
    // logging.logger.info('√ Compiling CSS completed.\n\n'.green.bold);

    if (result.warnings()) {
      logging.logger.info('There are some warnings with your build:\n'.yellow.bold);  
    }

    result.warnings().forEach(function (warn) {
      logging.logger.info(warn.toString());
    });

    logging.logger.debug('Css result:', result.css);

    logging.logger.info(['\n√ CSS processed to:', buildFilePath, '\n'].join(' ').green.bold);
...
Transpile.prepareFonts = function prepareFonts(appDirectory) {
  logging.logger.info('∆ Preparing fonts.\n'.yellow.bold);
  ...
  logging.logger.info('√ Fonts completed.\n'.green.bold);
}
