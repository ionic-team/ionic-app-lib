...
function buildAndroidDebug(appDirectory, jar, appId, options) {
...
function buildIOS(appDirectory, jar, appId, profile, buildMode, options) {
  return build(appDirectory, jar, appId, profile, {
    platform: 'ios',
...
function getBuild(appId, jar, buildId, extraQueryParams) {
  var q = Q.defer();
...
function downloadBuild(appId, jar, buildId, downloadDir) {
  return getBuild(appId, jar, buildId, { fields: ['url'] })
    .then(function(body) {
      var q = Q.defer();
      var build = body.data;
      var filename;
      ...
      filename = build.name + '.' + determineFileExtensionByPlatform(build.platform);
...
function determineFileExtensionByPlatform(platform) {
  switch (platform) {
  case 'android':
    return 'apk';
  case 'ios':
...
module.exports = {
  ...
  buildAndroidRelease: buildAndroidRelease,
  buildIOS: buildIOS,
  listBuilds: listBuilds,
  getBuild: getBuild,
  downloadBuild: downloadBuild,
  determineFileExtensionByPlatform: determineFileExtensionByPlatform
};
