...
IonicTask.prototype.fetchStarterTemplates = function() {
  ...
  // l.logger.info('About to fetch template');
  ...
  request({ url: downloadUrl, proxy: proxy }, function(err, res, html) {
    if(!err && res && res.statusCode === 200) {
      ...
      l.logger.error('Unable to fetch the starter templates. Please check your internet connection'.red);
      q.reject(res);
    }
  });
  return q.promise;
...
IonicTask.prototype.list = function list(templates) {
  ...
  _.each(templates, function(template) {
    ...
    var outStr = []
    l.logger.info(shortName.green, dots, template.description);
  })
}
...
IonicTask.prototype.run = function(ionic) {
  ...
  l.logger.info('Ionic Starter templates'.green);
  this.list(templates);
