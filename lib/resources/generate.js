...
function addIonicIcons(platform, forceAddResources) {
  ...
  try {
    ...
    .then(getConfigData)
    .then(function(data) {
      if(!data.widget.platform || !data.widget.platform[0].icon || (data.widget.platform.length == 1 && data.widget.platform[0]['$'].name != platform)) {
...
function copyIconFilesIntoResources(forceAddResources, appDirectory) {
  if (!appDirectory) {
    appDirectory = process.cwd(); //default to cwd if none are passed.
  }
...
function addPlatformIcons(platform, configData) {
  ...
  if(!configData.widget.platform) {
    ...
    //Theres a platform we need to look up and access, instead of blinding adding a platform element.
    for(var i = 0, j = configData.widget.platform.length; i < j; i++) {
      if(configData.widget.platform[i]['$'].name == platform) {
        platElem = configData.widget.platform[i];
        platElem.icon = platElem.icon || [];
        platElem.splash = platElem.splash || [];
      }
    }
  }

  // platElem = platElem || { '$': { name: platform }, icon: [], splash: [] };
