...
function handleApiSurvey(req, res) {
  ...
  getBody(req, function(body) {
    ...
    Login.retrieveLogin().then(function(jar) {
      ...
      request({
        method: 'POST',
        uri: settings.IONIC_DASH_API + 'survey/' + campaign,
        json: true,
        body: JSON.stringify(b.results),
        headers: {
      ...
      function(error, response, body) {
        if (error || parseInt(response.statusCode, 10) !== 200) {
          console.error('Saving survey response failed:', error || 'Server Error: ' + response.statusCode);
        } else {
          res.writeHead(200, {'Content-Type': 'application/json'});
          res.end('{"status": "success"}');
        }
      ...
      log.error('Unable to send survey results', err);
      res.writeHead(500, {'Content-Type': 'application/json'});
