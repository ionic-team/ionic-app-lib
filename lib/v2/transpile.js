...
Transpile.processSass = function processSass(appDirectory, entryPath, includePaths, outputPath) {
  var q = Q.defer();
  logging.logger.debug('Process sass appDirectory', appDirectory);
  logging.logger.debug('Process sass entryPath', entryPath);
  logging.logger.debug('Process sass includePaths', includePaths);
  logging.logger.debug('Process sass entryPath', outputPath);
  ...
  var resolvedEntryPath = path.resolve(path.join(appDirectory, entryPath));
  var resolvedIncludePaths = includePaths.map(function(path){ return path.resolve(path.join(appDirectory, path))});

  var sassOutput = sass.renderSync({file: resolvedEntryPath, includePaths: resolvedIncludePaths});
  ...
  var resolvedOutputPath = path.resolve(path.join(appDirectory, outputPath));
