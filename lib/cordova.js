...
Cordova.setupLiveReload = function() {
  ...
  l.logger.info('Setup Live Reload'.green.bold);
...
Cordova.addPlatform = function addPlatform(projectRoot, platform, savePlatform) {
  l.logger.debug('Cordova.addPlatform: ', projectRoot, platform, savePlatform);
  // var opts = { 
  ...
  .then(function() {
    ...
    l.logger.info(('Added ' + platform + ' platform').green);
    process.env.PWD = originalPwd;
    if (savePlatform) {
      l.logger.debug('Cordova.addPlatform call specified to save platform. Saving to package.json now.');
...
Cordova.addPlugin = function addPlugin(projectRoot, pluginId, pluginVariables, savePlugin) {
  l.logger.debug('Cordova.addPlugin: projectRoot', projectRoot, 'pluginId', pluginId, 'pluginVariables', pluginVariables, 'savePlugin', savePlugin);
  var originalPwd = process.env.PWD;
  process.env.PWD = projectRoot;
  ...
  .catch(function(error) {
    l.logger.error('Error occurred while adding plugin: ', error);
    throw error;
  });
};
