...
Setup.modifyIndexFile = function modifyIndexFile(appDirectory) {
  ...
  l.logger.debug('Setup.modifyIndexFile for ' + appDirectory);
  ...
  try {
    ...
    l.logger.info('Updated '.green.bold + indexPath.bold + ' <link href>'.yellow.bold + ' references to sass compiled css'.green.bold);

    l.logger.info('\nIonic project ready to use Sass!'.green.bold);
    l.logger.info(' * Customize the app using'.yellow.bold, 'scss/ionic.app.scss'.bold);
    l.logger.info(' * Run'.yellow.bold, 'ionic serve'.bold, 'to start a local dev server and watch/compile Sass to CSS'.yellow.bold);
    l.logger.info('');
...
Setup.sassSetup = function sassSetup(appDirectory) {
  ...
  if (!Utils.gulpInstalledGlobally()) {
    ...
    l.logger.info(gulpMessage);
    return Q(gulpMessage);
  }

  l.logger.debug('Setup.sassSetup for ' + appDirectory);
...
Setup.buildSass = function buildSass(appDirectory) {
  l.logger.debug('Setup.buildSass for ' + appDirectory);
  var q = Q.defer();
  ...
  childProcess.on('exit', function(code){
    ...
    if(code === 0) {
      l.logger.info('Successful '.green.bold + 'sass build'.bold);
      q.resolve();
    } else {
      l.logger.info('Error running '.error.bold + 'gulp sass'.bold);
      q.reject();
    }
  });
...
Setup.npmInstall = function npmInstall(appDirectory) {
  l.logger.debug('Setup.npmInstall at ' + appDirectory);
  var q = Q.defer();
  var prevDir = process.cwd();
  process.chdir(appDirectory);
  ...
  childProcess.on('exit', function(code){
    ...
    process.stderr.write('\n');
    if(code === 0) {
      l.logger.info('Successful '.green.bold + 'npm install'.bold);
      q.resolve();
    } else {
      l.logger.info('Error running '.error.bold + 'npm install'.bold);
      q.reject();
    }
  });
...
//   l.logger.info('Setup Facebook has not yet been implemented.'.red);
...
Setup.checkPreReqs = function checkPreReqs() {
  ...
  try {
    ...
    if (!platformsExist && semver.satisfies(envInfo.cordova, '<=4.3.0')) {
      l.logger.info('You are using a version of Cordova less than 4.3.0. There is a known issue if you add plugins before platforms are added.');
      l.logger.info('We highly suggest you add your platforms before running this command.');
      hasPreReqs = false;
    }
  } catch (ex) {
    l.logger.error('Error checking your Cordova CLI version: %s', ex, {});
  }
...
Setup.setupFacebook = function setupFacebook(appDirectory) {
  ...
  l.logger.info(installMessage.blue.bold);
  ...
  if (!Setup.checkPreReqs()) {
    l.logger.error('prereq check fail')
  ...
  .then(function() {
    l.logger.info('Installing Facebook Plugin'.info);
  ...
  .catch(function(ex) {
    l.logger.info('Error occurred: %s', ex, {});
    l.logger.info(ex.stack);
  });
};
...
Setup.showMarkup = function showMarkup() {
  ...
  l.logger.info('Put this JS in your Controller to use the Facebook plugin:'.green);
  l.logger.info(facebookSnippet.blue);
  l.logger.info('Use this HTML to trigger the above login method'.green);
  l.logger.info('<button ng-click="login()">Login to Facebook!</button>'.blue)
};
...
Setup.downloadZipToDir = function downloadZipToDir(downloadUrl, zipOutPath, outputDir) {
  l.logger.info('Attemping to download the Facebook plugin'.info);
  var q = Q.defer();
  ...
  if (fs.existsSync(outputDir)) {
    l.logger.info('The Facebook plugin has already been downloaded. Skipping this step.'.green);
    return q.resolve();
  }

  utils.fetchArchive(zipOutPath, downloadUrl)
  .then(function(data) {
    l.logger.info('The Facebook plugin has finished downloading.'.green);
    q.resolve();
  }, function(error) {
    l.logger.info('Failed to download Facebook plugin - ', error);
    q.reject();
  })
