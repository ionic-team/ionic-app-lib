...
Setup.sassSetup = function sassSetup() {
  var q = Q.defer();
  var self = this;
  // self.ionic = ionic;

  Setup.npmInstall().then(function(){

    var indexPath = path.resolve( path.join('www', utils.getContentSrc(process.cwd())) );
    ...
    try {
      ...
      q.reject(e);
      return utils.fail('Error parsing ' + indexPath + ': ' + e);
    }
...
Setup.buildSass = function buildSass() {
  var q = Q.defer();
...
Setup.npmInstall = function npmInstall() {
  var q = Q.defer();
