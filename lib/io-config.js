  ...
  fs.readFile(CONFIG_BACKUP, function(err, data) {
    if (err && !err.code === 'ENOENT') {
      ...
      if (!jsonObj['app_id'] || !jsonObj['api_key']) {
        safe = false;
        logging.logger.warn('No app ID detected, please run \n' + 'ionic upload'.yellow);
      }
      if (!jsonObj['api_key']) {
        safe = false;
        logging.logger.warn('No API key detected, please run \n' + 'ionic config set api_key your-api-key'.yellow);
      }
    ...
    fs.readFile(CONFIG_BACKUP, function(err, data) {
      ...
      fs.writeFile(CONFIG_BACKUP, JSON.stringify(jsonObj), function(error) {
        if (error) {
          ...
          if (key) {
            logging.logger.info("Saved " + key + ", writing to ionic-core.js...");
          } else {
            logging.logger.info('Building core config factory...')
          }
            ...
            if (available) {
              ...
              fs.readFile(CORE_FILE, function(er, content) {
                ...
                if (jsFile) {
                  ...
                  logging.logger.error('Outdated ionic-service-core, please install the latest version.'.red);
                  deferred.resolve(key);
                }
              });
            } else {
