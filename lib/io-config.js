...
IoConfig.listConfig = function listConfig() {
  fs.readFile(CONFIG_BACKUP, function(err, data) {
    if (err) {
      if (!err.code === 'ENOENT') {
        l.logger.error('Error reading config', err);
      }
    } else if (data) {
      jsonObj = JSON.parse(data);
      ...
      keys.forEach(function(item) {
        ...
        l.logger.info(item.key + item.value);
      });
    }
  });
}
...
IoConfig.warnMissingData = function warnMissingData() {
  ...
  fs.readFile(CONFIG_BACKUP, function(err, data) {
    if (err && !err.code === 'ENOENT') {
      ...
      if (!jsonObj['app_id']) {
        safe = false;
        l.logger.warn('No app ID detected, please run \n' + 'ionic upload'.yellow);
      }
      if (!jsonObj['api_key']) {
        safe = false;
        l.logger.warn('No API key detected, please run \n' + 'ionic config set api_key your-api-key'.yellow);
      }
...
IoConfig.writeIoConfig = function writeIoConfig(key, val, set) {
  ...
  if (ALLOWED_CONFIG[key] || key == false) {
    ...
    fs.readFile(CONFIG_BACKUP, function(err, data) {
      ...
      fs.writeFile(CONFIG_BACKUP, JSON.stringify(jsonObj), function(error) {
        if (error) {
          ...
          if (key) {
            l.logger.info("Saved " + key + ", writing to ionic-core.js...");
          } else {
            l.logger.info('Building core config factory...')
          }
            ...
            if (available) {
              fs.readFile(CORE_FILE, function(er, content) {
                ...
                if (slices.length === 3) {
                  ...
                  l.logger.error('Outdated ionic-service-core, please install the latest version.'.red);
                  deferred.resolve(key);
                }
              });
            } else {
    ...
    l.logger.error('Unauthorized configuration value'.red);
    ...
    fs.readFile(CONFIG_BACKUP, function(e, d) {
      ...
      if (!disable) {
          ...
          if (err) {
            ...
            $("script").each(function() {
              ...
              if ($(this).attr('src') === "cordova.js" && name === "ionic.service.deploy" && set) {
                ...
                l.logger.info("Deleting component from index.html");
                $(this).remove();
              } else if (set && $(this).attr('src') === ioPath) {
                exists = true;
              }
            ...
            if (set && !exists) {
              l.logger.info('Adding component to index.html');
        ...
        fs.readFile(APP_FILE, function(err, data) {
          if (err) {
            l.logger.error("ERROR: ", err);
            l.logger.error("Is your app declaration contained in 'app.js'?");
            deferred.reject(new Error(err));
          } else {
            ...
            jsFile = jsFile.replace('\"' + name + '\",', '');
            if (set) {
              l.logger.info('Injecting ' + name + ' into app.js');
              ...
              l.logger.info('Removing ' + name + ' from app.js');
            }
        ...
        l.logger.warn('Automatic file manipulation disabled, please manually inject components.'.yellow);
        deferred.resolve(true);
      }
    });
  }
...
IoConfig.initIoPlatform = function initIoPlatform(appDirectory, jar) {
  l.logger.info('Initializing app with ionic.io....'.green.bold);
  ...
  try {
    ...
    .catch(function(ex) {
      l.logger.error('An error occurred initializing the app: %s', ex, {});
      throw ex;
    });
  } catch(ex) {
    l.logger.error("Error initializing app: %s", ex, {});
  }
}
...
IoConfig.dashInit = function dashInit(project, jar) {
  ...
  l.logger.debug('Getting app information from ', settings.IONIC_DASH);
