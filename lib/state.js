...
State.getPackageJson = function getPackageJson(appDirectory) {
  ...
  try {
    ...
    l.logger.error('There was an error opening your package.json file.');
    l.logger.error(ex);
    Utils.fail(ex);
  }
...
State.saveState = function saveState(appDirectory) {
  l.logger.info('Saving your Ionic app state of platforms and plugins'.blue.bold);
  var packageJson = State.getPackageJson(appDirectory);
  try {
    State.saveExistingPlatforms(appDirectory, packageJson);
    l.logger.info('Saved platform'.green);
    State.saveExistingPlugins(appDirectory, packageJson);
    l.logger.info('Saved plugins'.green);
    State.savePackageJson(appDirectory, packageJson);
    l.logger.info('Saved package.json'.green);
  } catch (ex) {
    l.logger.error('There was an error saving your current Ionic setup'.red);
    l.logger.error(ex);
  }
};
...
State.saveExistingPlatforms = function saveExistingPlatforms(appDirectory, packageJson) {
  ...
  platforms.forEach(function(platform) {
    ...
    try {
      ...
      l.logger.info('There was an error trying to save your existing state', ex);
    }
  });
};
...
State.saveExistingPlugins = function saveExistingPlugins(appDirectory, packageJson) {
  ...
  // l.logger.info('config xml path ', configXmlPath)
  ...
  if (fetchJson) {
    // l.logger.info('fetchJson', fetchJson)
    ...
    for (pluginId in fetchJson) {
      ...
      try {
        ...
        l.logger.info(('Plugin ' + pluginId + ' does not exist in the plugins directory. Skipping').yellow);
        continue;
      }
      ...
      // l.logger.info('plugin.source.type', plugin.source.type);
      ...
      if (preferences && preferences.length > 0) {
        ...
        preferences.forEach(function(preference) {
          ...
          // l.logger.info('looking at preference: ', preference)
        })
      }
      ...
      if (hasVariables) {
        // l.logger.info('we have avariables to look at:', variableList)
        // var features = configXmlData.widget.feature;
        // l.logger.info('features', features)
        ...
        // l.logger.info('feature found:', feature);
        // var featureParams = feature.param;

        variableList.forEach(function(variable) {
          // l.logger.info('Looking up variable:', variable)
        ...
        // l.logger.info('We have the plugin parameters with values:', keyValueList)
    ...
    l.logger.info('There was no fetch.json file available to restore plugin information from.'.red.bold);
    l.logger.info('Restoring plugins from scanning the plugins folder is still being worked on.'.red.bold);
  }
...
State.getPlatformVersion = function getPlatformVersion(path) {
  ...
  // l.logger.info('Version for path:', path, version)
...
State.addOrUpdatePlatformToPackageJson = function addOrUpdatePlatformToPackageJson(packageJson, platformId, platformInfo) {
  ...
  if (!existingPlatform) {
    ...
    l.logger.info('A platform already exists - now updating the entry:', existingPlatform, platformInfo);
    packageJson.cordovaPlatforms[existingPlatformIndex] = platformInfo;
  }


  // l.logger.info('platformExists:', platformExists)
...
State.savePlatform = function savePlatform(appDirectory, locator) {
  l.logger.debug('State.savePlatform', appDirectory, locator);
  // Locator may be:
  // Name:        ios, android
  ...
  // l.logger.info('platform args:', platformArgs);
...
State.savePackageJson = function savePackageJson(appDirectory, packageJsonData) {
  l.logger.debug('State.savePackageJson', appDirectory, packageJsonData);
  try {
    var packageJsonPath = path.join(appDirectory, 'package.json');
    fs.writeFileSync(packageJsonPath, JSON.stringify(packageJsonData, null, 2));
  } catch (e) {
    l.logger.error(('Error saving ' + packageJsonPath + ': %s').bold, e, {});
  }
};

State.restoreState = function restoreState(appDirectory, options) {

  l.logger.debug('State.restoreState', appDirectory, options);
  ...
  if (!options.platforms && !options.plugins) {
    l.logger.error('You must specify either platforms or plugins to restore the state of your Ionic application.');
    return;
  }

  l.logger.info('Attempting to restore your Ionic application from package.json\n'.yellow.bold);
  ...
  if (options.platforms) {
    l.logger.info('Restoring Platforms\n'.blue.bold)
  ...
  .then(function() {
    if (options.plugins) {
      l.logger.info('\rRestoring Plugins\n'.blue.bold)
      return State.restorePlugins(appDirectory, packageJson)
      .then(function(){
        l.logger.info('Restore plugins is complete\n'.green.bold);
      });
    } else {
      return Q();
    }
  ...
  .then(function() {
    l.logger.info('Ionic state restore completed\n'.blue.bold)
  })
  .catch(function(ex) {
    l.logger.error('Ionic state restore failed\n'.red.bold)
    l.logger.error(ex);
  });
};

State.restorePlugins = function restorePlugins(appDirectory, packageJson) {
  l.logger.debug('State.restorePlugins', appDirectory, packageJson);
  var q = Q.defer();
...
State.restorePlatforms = function restorePlatforms(appDirectory, packageJson) {
  l.logger.debug('State.restorePlatforms', appDirectory, packageJson);
  var q = Q.defer();
...
State.processPlatform = function processPlatform(appDirectory, index, packageJson, promise) {
  ...
  try {
    // l.logger.info('processing platform', index, packageJson)
    ...
    l.logger.info(platformCommand);
    exec(platformCommand, function() {
        State.processPlatform(appDirectory, index + 1, packageJson, promise);
    });
  } catch (ex) {
    l.logger.error('An error happened processing the previous cordova plugins')
    l.logger.error(ex)
    promise.reject(ex)
  }
};
...
State.createAddRemoveStatement = function createAddRemoveStatement(plugin) {
    // l.logger.info('Creating add/remove statement', plugin)
    try {
      ...
      l.logger.error('Failed to create add plugin statement: %s', ex, {})
    }

    // l.logger.info('plugin cmd', pluginCmd)
...
State.removePlatform = function removePlatform(appDirectory, platform) {
  // l.logger.info('Args:', platformArgs);
...
State.savePlugin = function savePlugin(appDirectory, pluginId, variables) {
  l.logger.debug('State.savePlugin - appDirectory:', appDirectory, 'pluginId', pluginId, 'variables', variables);
  ...
  State.addOrUpdatePluginToPackageJson(packageJson, pluginId, pluginInfo);
  l.logger.info('Save plugin to package.json completed');
...
State.removePlugin = function removePlugin(appDirectory, pluginId) {
  l.logger.debug('State.removePlugin - ', appDirectory, pluginId);
  var packageJson = State.getPackageJson(appDirectory);
  for (var i = 0, j = packageJson.cordovaPlugins.length; i < j; i++) {
    if (typeof packageJson.cordovaPlugins[i] == 'string' && packageJson.cordovaPlugins[i] == pluginId) {
...
State.saveXmlFile = function saveXmlFile(xmlData, xmlPath) {
  try {
    ...
    l.logger.error('Could not save your xml file to path:', xmlPath);
  }
};
...
State.checkAndSaveConfigXml = function checkAndSaveConfigXml(appDirectory, pluginId) {
  ...
  // l.logger.info('xml data:', pluginXmlData);
...
State.resetState = function resetState(appDirectory, options) {
  ...
  l.logger.info('Removed platforms and plugins'.blue.bold);
  State.restoreState(appDirectory, options)
  .then(function() {
    l.logger.info('Ionic reset state complete'.green.bold);
  })
};

State.clearState = function clearState(appDirectory) {
  l.logger.info('Clearing out your Ionic app of platforms, plugins, and package.json entries'.blue.bold);
  var platformPath = path.join(appDirectory, 'platforms');
  var pluginPath = path.join(appDirectory, 'plugins');
  shelljs.rm('-rf', [platformPath, pluginPath]);
  ...
  State.savePackageJson(appDirectory, packageJson);
  l.logger.info('Ionic app state cleared'.green.bold);
};
