...
function readInPackageJson(jsonPath) {
  return require(jsonPath);
}

function getPackageJson(appDirectory) {
  ...
  try {
    ...
    packageJson = readInPackageJson(packageJsonPath);
...
function getPackageJsonReadStream(appDirectory) {
  var packageJsonPath = path.join(appDirectory, 'package.json');
  return fs.createReadStream(packageJsonPath);
}

function addOrUpdatePluginToPackageJson(packageJson, pluginId, pluginInfo) {
  var existingPlugin;
...
function saveState(appDirectory) {
  log.info('Saving your Ionic app state of platforms and plugins');
  var packageJson = getPackageJson(appDirectory);
  try {
    saveExistingPlatforms(appDirectory, packageJson);
    log.info('Saved platform');
    saveExistingPlugins(appDirectory, packageJson);
    log.info('Saved plugins');
    savePackageJson(appDirectory, packageJson);
...
function platformExists(appDirectory, platform) {
...
function saveExistingPlatforms(appDirectory, packageJson) {
  ...
  platforms.forEach(function(platform) {
    ...
    try {
      ...
      var version = getPlatformVersion(versionPath);
...
function saveExistingPlugins(appDirectory, packageJson) {
  ...
  if (fetchJson) {
    ...
    for (pluginId in fetchJson) {
      if (fetchJson.hasOwnProperty(pluginId)) {
        ...
        pluginXml = getXmlData(pluginXmlPath);
        ...
        if (hasVariables) {
          ...
          var pluginPreferences = getPluginPreferences(fetchJson, pluginId);
...
function getPlatformVersion(path) {
...
function addOrUpdatePlatformToPackageJson(packageJson, platformId, platformInfo) {
...
function savePlatform(appDirectory, locator) {
  ...
  if (locator === 'ios' || locator === 'android') {
    platform = locator;
    addOrUpdatePlatformToPackageJson(packageJson, platform);
    return savePackageJson(appDirectory, packageJson);
  }
  ...
  addOrUpdatePlatformToPackageJson(packageJson, platform, platformInfo);
  savePackageJson(appDirectory, packageJson);
}

function savePackageJson(appDirectory, packageJsonData) {
...
function restoreState(appDirectory, options) {
  ...
  if (options.platforms) {
    ...
    restorePromise = restorePlatforms(appDirectory, packageJson)
    .then(function() {
  ...
  .then(function() {
    if (options.plugins) {
      ...
      return restorePlugins(appDirectory, packageJson)
      .then(function() {
...
function restorePlugins(appDirectory, packageJson) {
  log.debug('State.restorePlugins', appDirectory, packageJson);
  var q = Q.defer();
  processPlugin(appDirectory, 0, packageJson, q);
  return q.promise;
}

function restorePlatforms(appDirectory, packageJson) {
  log.debug('State.restorePlatforms', appDirectory, packageJson);
  var q = Q.defer();

  processPlatform(appDirectory, 0, packageJson, q);

  return q.promise;
}

function processPlatform(appDirectory, index, packageJson, promise) {
  ...
  try {
    ...
    shelljs.exec(platformCommand, function() {
      processPlatform(appDirectory, index + 1, packageJson, promise);
    });
  } catch (ex) {
...
function processPlugin(appDirectory, index, packageJson, promise) {
  ...
  try {
    ...
    shelljs.exec(pluginCommand, { async: true }, function(code, output) {
      ...
      processPlugin(appDirectory, index + 1, packageJson, promise);
    });
  } catch (ex) {
    log.error('An error happened processing the previous cordova plugins');
    log.error(ex);
...
function removePlatform(appDirectory, platform) {
...
function getPluginParameters(configXmlData, pluginName) {
...
function getPluginPreferences(fetchJson, pluginName) {
...
*/
function savePlugin(appDirectory, pluginId, variables) {
  ...
  if (pluginId.indexOf('/') === -1) { // its just an id
    ...
    // Check and save for variables
    if (!variables) {
      addOrUpdatePluginToPackageJson(packageJson, pluginId);
    } else {
      pluginInfo.locator = pluginId;
      pluginInfo.id = pluginId = getPluginFromFetchJsonByLocator(appDirectory, pluginInfo.locator);
      addOrUpdatePluginToPackageJson(packageJson, pluginId, pluginInfo);
    }
...
function removePlugin(appDirectory, pluginId) {
...
function saveXmlFile(xmlData, xmlPath) {
  try {
    var xml2js = require('xml2js');
...
function getPluginFromFetchJsonByLocator(appDirectory, pluginLocator) {
...
function resetState(appDirectory, options) {
  ...
  restoreState(appDirectory, options)
  .then(function() {
    log.info('Ionic reset state complete');
  });
}

function clearState(appDirectory) {
...
module.exports = {
  readInPackageJson: readInPackageJson,
  getPackageJson: getPackageJson,
  getPackageJsonReadStream: getPackageJsonReadStream,
  addOrUpdatePluginToPackageJson: addOrUpdatePluginToPackageJson,
  saveState: saveState,
  platformExists: platformExists,
  saveExistingPlatforms: saveExistingPlatforms,
  saveExistingPlugins: saveExistingPlugins,
  getXmlData: getXmlData,
  getPlatformVersion: getPlatformVersion,
  addOrUpdatePlatformToPackageJson: addOrUpdatePlatformToPackageJson,
  savePlatform: savePlatform,
  savePackageJson: savePackageJson,
  restoreState: restoreState,
  restorePlugins: restorePlugins,
  restorePlatforms: restorePlatforms,
  processPlatform: processPlatform,
  createAddRemoveStatement: createAddRemoveStatement,
  processPlugin: processPlugin,
  removePlatform: removePlatform,
  getPluginParameters: getPluginParameters,
  getPluginPreferences: getPluginPreferences,
  savePlugin: savePlugin,
  removePlugin: removePlugin,
  saveXmlFile: saveXmlFile,
  getPluginFromFetchJsonByLocator: getPluginFromFetchJsonByLocator,
  resetState: resetState,
  clearState: clearState,
  getPluginsFromFetchJson: getPluginsFromFetchJson
};
