...
Login.requestLogIn = function requestLogin(email, password, saveCookies) {
  ...
  request({
    ...
    if (response.statusCode != 200) {
      // return utils.fail('Email or Password incorrect. Please visit '+ settings.IONIC_DASH.white +' for help.'.red);
...
Login.handleResponse = function handleResponse(err, response, body) {
  ...
  if(saveCookies) {
    ...
    self.cookieData.set(settings.IONIC_DASH, jar.getCookies(settings.IONIC_DASH));
    self.cookieData.save();

    transformedCookies = self.cookieData.get(settings.IONIC_DASH);
  } else {
    try {
      var jsonString = JSON.stringify(jar.getCookies(settings.IONIC_DASH, null, 2));
      transformedCookies = JSON.parse(jsonString);
