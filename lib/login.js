...
function retrieveLogin() {
  ...
  try {
    ...
    var tempJar = cookieData.get(settings.IONIC_DASH);

    if (tempJar && tempJar.length) {
      for (var i in tempJar) {
        if (tempJar.hasOwnProperty(i)) {
          var cookie = tempJar[i];
...
function requestLogIn(email, password, saveCookies) {
  var q = Q.defer();
  ...
  request({
    ...
    if (saveCookies) {
      transformedCookies = saveCookies(jar);
    } else {
      try {
...
function getDownloadLink(jar, appId) {
  var q = Q.defer();
