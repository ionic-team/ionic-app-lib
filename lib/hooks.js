...
function addCliHookDirectory(appDirectory, cliHookPath, hookDirectoryName) {
  fs.readdir(cliHookPath, function(err, files) {
    ...
    for (var x = 0; x < files.length; x += 1) {
      ...
      addHookScript(appDirectory, cliHookPath, hookDirectoryName, hookFilename);
    }
  });
}

function addHookScript(appDirectory, cliHookPath, hookDirectoryName, hookFilename) {
...
function add(appDirectory) {
  ...
  fs.readdir(cliHooksPath, function(err, files) {
    ...
    for (var x = 0; x < files.length; x += 1) {
      ...
      addCliHookDirectory(appDirectory, path.join(cliHooksPath, files[x]), files[x]);
    }
  });
...
function setHooksPermission(appDirectory) {
...
module.exports = {
  addCliHookDirectory: addCliHookDirectory,
  addHookScript: addHookScript,
  add: add,
  remove: remove,
  setHooksPermission: setHooksPermission
};
