  ...
  try {
    ...
    if( !fs.existsSync(projectScript) ) {
      ...
      try {
        ...
        l.logger.error('addCliHookDirectory fs.createReadStream: %'.error, e, {});
        return;
      }
    }
    ...
    try {
      ...
      l.logger.error('addCliHookDirectory fs.chmodSync: %s'.error, e, {});
    }

  } catch(e) {
    l.logger.error('Error adding hook script ' + hookDirectoryName + '/' + hookFilename + ', %s', e, {});
  }
};

Hooks.add = function add(appDirectory) {
  l.logger.info('Adding in default Ionic Cordova hooks'.blue.bold);
  ...
  l.logger.info('Added default Ionic Cordova hooks'.green.bold)

};

Hooks.remove = function remove(appDirectory) {
  l.logger.info('Removing the Ionic Cordova plugin hooks'.blue.bold)
  var oldPluginHooks = [
  ...
  l.logger.info('Removed the Ionic Cordova hooks'.green.bold);
    ...
    hooksDirs.forEach(function(hookDir) {
      ...
      hooksInDir.forEach(function(hook){
        ...
        try {
          ...
          l.logger.error('Hooks.setHooksPermission fs.chmodSync: %s'.error, e, {});
        }
      })
    });
  } catch (ex) {
    l.logger.error('Error: %s', ex);
  }

  l.logger.info('Updated the hooks directory to have execute permissions'.green);
};
