...
Hooks.logger = logging.defaultLogger;
  ...
  try {
    ...
    if( !fs.existsSync(projectScript) ) {
      ...
      try {
        ...
        Hooks.logger.error('addCliHookDirectory fs.createReadStream: %'.error, e, {});
        return;
      }
    }
    ...
    try {
      ...
      Hooks.logger.error('addCliHookDirectory fs.chmodSync: %s'.error, e, {});
    }

  } catch(e) {
    Hooks.logger.error('Error adding hook script ' + hookDirectoryName + '/' + hookFilename + ', %s', e, {});
  }
};

Hooks.add = function add(appDirectory) {
  Hooks.logger.info('Adding in default Ionic Cordova hooks'.blue.bold);
  ...
  Hooks.logger.info('Added default Ionic Cordova hooks'.green.bold)

};

Hooks.remove = function remove(appDirectory) {
  Hooks.logger.info('Removing the Ionic Cordova plugin hooks'.blue.bold)
  var oldPluginHooks = [
  ...
  Hooks.logger.info('Removed the Ionic Cordova hooks'.green.bold);
    ...
    hooksDirs.forEach(function(hookDir) {
      ...
      hooksInDir.forEach(function(hook){
        ...
        try {
          ...
          Hooks.logger.error('Hooks.setHooksPermission fs.chmodSync: %s'.error, e, {});
        }
      })
    });
  } catch (ex) {
    Hooks.logger.error('Error: %s', ex);
  }

  Hooks.logger.info('Updated the hooks directory to have execute permissions'.green);
};
