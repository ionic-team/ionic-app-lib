...
Project.PROJECT_FILE = 'ionic.json';
...
Project.load = function load(appDirectory) {
  ...
  try {
    ...
    if (e instanceof SyntaxError) {
      console.error('Uh oh! There\'s a syntax error in your ionic.json file:\n' + e.stack);
      process.exit(1);
    }
  }
  if (!found) {
    try {
      ...
      console.warn('WARN: ionic.project has been renamed to ionic.json, please rename it :)')
      found = true;
    } catch(e) {
      if (e instanceof SyntaxError) {
  ...
  if (!found) {
    ...
    if (fs.existsSync(oldV2ProjectFile)) {
      console.warn('WARN: ionic.config.js has been deprecated in favor of ionic.json.');
      console.info('Creating default ionic.json for you now...\n');
      data.v2 = true;
