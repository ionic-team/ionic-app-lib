...
Start.promptLogin = function() {
  ...
  if(ionicConfig) {
    ...
    if(didPrompt == 'y') {
      logging.logger.info('New!'.bold + ' Add push notifications, update your app remotely, and package iOS and Android apps with the Ionic Platform!\n'.bold + IONIC_DASH + '/signup');
      return;
    }
...
Start.printQuickHelp = function(options) {
  ...
  logging.logger.info('\nTo run on iOS:'.bold);
  logging.logger.info('  ionic platform add ios');
  logging.logger.info('  ionic run ios');
  logging.logger.info('\nTo run on Android:'.bold);
  logging.logger.info('  ionic platform add android');
  ...
  logging.logger.info('\nFor everything else, please read the Getting Started guide:\n' + 'http://ionicframework.com/docs/v2/getting-started/'.bold);
  logging.logger.info('\n');
