...
Start.runExecCommand = function runExecCommand(command) {
  ...
  childProcess.exec(command, function(err, stdout, stderr) {
    if (err) {
      Utils.fail('Unable to run exec command');
      q.reject(err);
    } else {
      q.resolve(stdout);
    }
...
Start.createCordovaApp = function createCordovaApp(options) {
  var q = Q.defer();
  //Do cordova create
  var childProcess = require('child_process');
  var cordovaCreateCmd = ['cordova create', options.targetPath, options.appName];

  if (options.ionicAppId) {
    cordovaCreateCmd.push(options.ionicAppId);
  }

  childProcess.exec(cordovaCreateCmd.join(' '), function(err, stdout, stderr) {
    if (err) {
      Utils.fail('Unable to create cordova application');
      q.reject(err);
    } else {
      q.resolve(stdout);
    }
...
Start.startV2App = function startV2App(options) {
  ...
  var repoFolderName = 'ionic2-starter-master';
  var repoUrl = 'https://github.com/driftyco/ionic2-starter/archive/master.zip';
  // var repoUrl = 'https://ionic-io-assets.s3.amazonaws.com/lab-test/ionic2-starter-master.zip';

  return Start.createCordovaApp(options)
