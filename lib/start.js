...
Start.createCordovaApp = function createCordovaApp(options) {
  return Utils.fetchArchive(options.targetPath, 'https://github.com/apache/cordova-app-hello-world/archive/master.zip')
  .then(function(result) {
    cp('-R', options.targetPath + '/cordova-app-hello-world-master' + '/*', options.targetPath);
    rm('-rf', options.targetPath + '/cordova-app-hello-world-master');
    rm('-rf', options.targetPath + '/CONTRIBUTING.md');
    rm('-rf', options.targetPath + '/LICENSE');
    rm('-rf', options.targetPath + '/NOTICE');
    rm('-rf', options.targetPath + '/README.md');
    rm('-rf', options.targetPath + '/RELEASENOTES.md');
    rm('-rf', options.targetPath + '/config.xml');
    rm('-rf', options.targetPath + '/index.js');
    rm('-rf', options.targetPath + '/package.json');
    rm('-rf', options.targetPath + '/www');
    rm('-rf', options.targetPath + '/res');
  });
};
