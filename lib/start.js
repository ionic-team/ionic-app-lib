...
Start.startApp = function startApp(options) {
  ...
  .catch(function(err) {
    l.logger.error('Error Initializing app: %s', err, {});
...
Start.addDefaultPlatforms = function addDefaultPlatforms(options) {
  ...
  if (currOs == 'darwin') {
    l.logger.info('Adding in iOS application by default'.yellow);
...
Start.loadAppSetup = function loadAppSetup(options) {
  ...
  if(fs.existsSync(appJsonPath)) {
    try {
      ...
      l.logger.error('app.json error: %s', e, {});
    }
  }
...
Start.fetchCreatorApp = function(options) {
  ...
  request({ url: downloadUrl, proxy: proxy }, function(err, res, html) {
    if(!err && res && res.statusCode === 200) {
      ...
      // l.logger.info(['Unable to fetch', err, res.statusCode].join(' '));
      q.reject(res);
    }
    q.resolve();
  });
...
Start.fetchCodepen = function(options) {
  ...
  l.logger.info('Downloading Codepen: ', codepenUrl.green.bold);
...
Start.fetchGithubStarter = function(options, repoUrl) {
  ...
  if(!urlParse.hostname || urlParse.hostname.toLowerCase() !== 'github.com' || pathSplit.length !== 2) {
    l.logger.error(('Invalid Github URL: ' + repoUrl).error );
    l.logger.error(('Example of a valid URL: https://github.com/driftyco/ionic-starter-tabs/').error );
    utils.fail('');
    q.reject();
    return q.promise;
  }
  ...
  utils.fetchArchive(options.targetPath, repoUrl).then(function() {
    ...
    l.logger.error('Please verify you are using a valid URL or a valid ionic starter.');
    l.logger.error('View available starter templates: `ionic templates`');
    l.logger.error('More info available at: \nhttp://ionicframework.com/getting-started/\nhttps://github.com/driftyco/ionic-cli');
    return utils.fail('');
  });
...
Start.fetchZipStarter = function fetchZipStarter(options) {
  ...
  l.logger.info('Fetching ZIP from url:', options.zipFileDownload.bold, 'to: ', options.targetPath);
...
Start.initCordovaFromGui = function initCordovaFromGui(options, appSetup) {
  ...
  l.logger.debug('Initializing Cordova for Gui');

  try {
    if (options.isCordovaProject) {
      Hooks.setHooksPermission(options.targetPath);
      l.logger.info('Update Config.xml'.green.bold);
      ...
      l.logger.info('Initializing cordova project'.green.bold);
    ...
    l.logger.debug('Exception caught in initCordova: %s', ex, {});
    l.logger.debug('Exception details: %s', ex.stack, {});
    q.reject(ex);
  }
...
Start.updateConfigXml = function(targetPath, packageName, appName, ios, android) {
  try {
    ...
    l.logger.error('Error updating config.xml file: %s', e, {})
    return
  }
};
...
Start.updateLibFiles = function(libPath) {
  ...
  if( fs.existsSync(libSymlinkPath) ) {
    ...
    l.logger.info('Create www/lib/ionic/css symlink to ' + libCssSymlinkPath);
    fs.symlinkSync(libCssSymlinkPath, wwwIonicCssPath);

    var libJsSymlinkPath = path.join(libSymlinkPath, 'js');
    l.logger.info('Create www/lib/ionic/js symlink to ' + libJsSymlinkPath);
    fs.symlinkSync(libJsSymlinkPath, wwwIonicJsPath);

    var libFontsSymlinkPath = path.join(libSymlinkPath, 'fonts');
    l.logger.info('Create www/lib/ionic/fonts symlink to ' + libFontsSymlinkPath);
  ...
  try {
    l.logger.info('Replacing Ionic lib references with ' + libPath);
...
Start.finalize = function(options) {
  try {
    ...
    l.logger.error('There was an error finalizing the package.json file. %s', e, {});
  }
  ...
  try {
    ...
    l.logger.debug('Saved project file');
  } catch(e) {
    l.logger.error('Error saving project file');
  }
...
Start.printQuickHelp = function(options) {
  ...
  if(options.isCordovaProject) {
    ...
    l.logger.info('   Note: iOS development requires OS X currently'.small);
    l.logger.info('   See the Android Platform Guide for full Android installation instructions:'.small);
    l.logger.info('   https://cordova.apache.org/docs/en/edge/guide_platforms_android_index.md.html'.small);
...
Start.promptForOverwrite = function promptForOverwrite(targetPath) {
  ...
  l.logger.info('The directory'.error.bold, targetPath, 'already exists.'.error.bold);
  l.logger.info('Would you like to overwrite the directory with this new project?');
