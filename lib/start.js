...
function startApp(options) {
  ...
  .then(function() {
    if (options.isCordovaProject) {
      return updateConfigXml(options.targetPath, options.packageName,
...
function fetchWrapper(options) {
  ...
  var branchName = (options.v2 && options.typescript) ? 'typescript' : 'master';
  var repoUrl = 'https://github.com/driftyco/' + wrapperRepoName + '/archive/' + branchName + '.zip';

  Utils.fetchArchive(options.targetPath, repoUrl)
  .then(function() {
    var repoFolderName = wrapperRepoName + '-' + branchName;
...
function fetchSeed(options) {
  ...
  if ((options.template.indexOf('/') > -1 || options.template.indexOf('\\') > -1) &&
    ...
    seedType = 'local';
    return fetchLocalStarter(options);
  }

  // Ionic Github Repo
  seedType = 'ionic-starter';
  return fetchIonicStarter(options);
}

function loadAppSetup(options) {
...
function fetchCreatorApp(options) {
...
function convertTemplates(html, targetPath) {
  var templates = [];
...
function fetchLocalStarter(options) {
  var q = Q.defer();
...
function fetchIonicStarter(options) {
  var repoName;
  if (options.v2) {
  ...
  return fetchGithubStarter(options, repoUrl);
}


function fetchGithubStarter(options, repoUrl) {
  var q = Q.defer();
  ...
  var branchName = (options.v2 && options.typescript) ? 'typescript' : 'master';
  var repoFolderName = repoName + '-' + branchName;

  // ensure there's an ending /
  if (repoUrl.substr(repoUrl.length - 1) !== '/') {
    repoUrl += '/';
  }
  repoUrl += 'archive/' + branchName + '.zip';

  Utils.fetchArchive(options.targetPath, repoUrl).then(function() {

    try {
      if (options.v2) {
        shelljs.cp('-Rf', options.targetPath + '/' + repoFolderName + '/.', options.targetPath);
      } else {
...
function fetchZipStarter(options) {
  var q = Q.defer();
...
function initCordovaFromGui(options, appSetup) {
  var q = Q.defer();
...
function initCordovaNoCli(options, appSetup) {
  try {
...
function initCordova(options, appSetup) {
  var q;
  try {
    if (!options.isCordovaProject) {
    ...
    if (!Utils.cordovaInstalled()) {
      ...
      return initCordovaNoCli(options, appSetup);
    }
...
function updateConfigXml(targetPath, packageName, appName) {
  var q = Q.defer();
...
function updateLibFiles(libPath) {
...
function printQuickHelp(options) {
...
function promptForOverwrite(targetPath) {
  var q = Q.defer();
...
module.exports = {
  runSpawnCommand: runSpawnCommand,
  startApp: startApp,
  addDefaultPlatforms: addDefaultPlatforms,
  fetchWrapper: fetchWrapper,
  fetchSeed: fetchSeed,
  loadAppSetup: loadAppSetup,
  fetchCreatorApp: fetchCreatorApp,
  fetchCodepen: fetchCodepen,
  convertTemplates: convertTemplates,
  fetchLocalStarter: fetchLocalStarter,
  fetchIonicStarter: fetchIonicStarter,
  fetchGithubStarter: fetchGithubStarter,
  fetchZipStarter: fetchZipStarter,
  fetchPlnkr: fetchPlnkr,
  initCordovaFromGui: initCordovaFromGui,
  initCordovaNoCli: initCordovaNoCli,
  initCordova: initCordova,
  updateConfigXml: updateConfigXml,
  updateLibFiles: updateLibFiles,
  promptLogin: promptLogin,
  finalize: finalize,
  printQuickHelp: printQuickHelp,
