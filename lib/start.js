...
Start.logger = logging.defaultLogger;
...
Start.startApp = function startApp(options) {
  ...
  Start.logger.info(createMessage);
  ...
  .catch(function(err) {
    Start.logger.error('Error Initializing app: %s', err, {});
...
Start.addDefaultPlatforms = function addDefaultPlatforms(options) {
  ...
  if (currOs == 'darwin') {
    Start.logger.info('Adding in iOS application by default'.yellow);
...
Start.loadAppSetup = function loadAppSetup(options) {
  ...
  if(fs.existsSync(appJsonPath)) {
    try {
      ...
      Start.logger.error('app.json error: %s', e, {});
    }
  }
...
Start.fetchCreatorApp = function(options) {
  ...
  request({ url: downloadUrl, proxy: proxy }, function(err, res, html) {
    if(!err && res && res.statusCode === 200) {
      ...
      // Start.logger.info(['Unable to fetch', err, res.statusCode].join(' '));
      q.reject(res);
    }
    q.resolve();
  });
...
Start.fetchCodepen = function(options) {
  ...
  Start.logger.info('Downloading Codepen: ', codepenUrl.green.bold);
...
Start.fetchGithubStarter = function(options, repoUrl) {
  ...
  if(!urlParse.hostname || urlParse.hostname.toLowerCase() !== 'github.com' || pathSplit.length !== 2) {
    Start.logger.error(('Invalid Github URL: ' + repoUrl).error );
    Start.logger.error(('Example of a valid URL: https://github.com/driftyco/ionic-starter-tabs/').error );
    utils.fail('');
    q.reject();
    return q.promise;
  }
  ...
  utils.fetchArchive(options.targetPath, repoUrl).then(function() {
    ...
    Start.logger.error('Please verify you are using a valid URL or a valid ionic starter.');
    Start.logger.error('View available starter templates: `ionic templates`');
    Start.logger.error('More info available at: \nhttp://ionicframework.com/getting-started/\nhttps://github.com/driftyco/ionic-cli');
    return utils.fail('');
  });
...
Start.fetchZipStarter = function fetchZipStarter(options) {
  ...
  Start.logger.info('Fetching ZIP from url:', options.zipFileDownload.bold, 'to: ', options.targetPath);
...
Start.initCordovaFromGui = function initCordovaFromGui(options, appSetup) {
  ...
  Start.logger.debug('Initializing Cordova for Gui');

  try {
    if (options.isCordovaProject) {
      Hooks.setHooksPermission(options.targetPath);
      Start.logger.info('Update Config.xml'.green.bold);
      ...
      Start.logger.info('Initializing cordova project'.green.bold);
    ...
    Start.logger.debug('Exception caught in initCordova: %s', ex, {});
    Start.logger.debug('Exception details: %s', ex.stack, {});
    q.reject(ex);
  }
...
Start.updateConfigXml = function(targetPath, packageName, appName, ios, android) {
  try {
    ...
    Start.logger.error('Error updating config.xml file: %s', e, {})
    return
  }
};
...
Start.updateLibFiles = function(libPath) {
  ...
  if( fs.existsSync(libSymlinkPath) ) {
    ...
    Start.logger.info('Create www/lib/ionic/css symlink to ' + libCssSymlinkPath);
    fs.symlinkSync(libCssSymlinkPath, wwwIonicCssPath);

    var libJsSymlinkPath = path.join(libSymlinkPath, 'js');
    Start.logger.info('Create www/lib/ionic/js symlink to ' + libJsSymlinkPath);
    fs.symlinkSync(libJsSymlinkPath, wwwIonicJsPath);

    var libFontsSymlinkPath = path.join(libSymlinkPath, 'fonts');
    Start.logger.info('Create www/lib/ionic/fonts symlink to ' + libFontsSymlinkPath);
  ...
  try {
    Start.logger.info('Replacing Ionic lib references with ' + libPath);
...
Start.finalize = function(options) {
  try {
    ...
    Start.logger.error('There was an error finalizing the package.json file. %s', e, {});
  }
  ...
  try {
    ...
    Start.logger.debug('Saved project file');
  } catch(e) {
    Start.logger.error('Error saving project file');
  }
...
Start.printQuickHelp = function(options) {
  ...
  if(options.isCordovaProject) {
    ...
    Start.logger.info('   Note: iOS development requires OS X currently'.small);
    Start.logger.info('   See the Android Platform Guide for full Android installation instructions:'.small);
    Start.logger.info('   https://cordova.apache.org/docs/en/edge/guide_platforms_android_index.md.html'.small);
...
Start.promptForOverwrite = function promptForOverwrite(targetPath) {
  ...
  Start.logger.info('The directory'.error.bold, targetPath, 'already exists.'.error.bold);
  Start.logger.info('Would you like to overwrite the directory with this new project?');
