...
Start.addDefaultPlatforms = function addDefaultPlatforms(options) {
  ...
  if (currOs == 'darwin') {
    ...
    return Cordova.addPlatform(options.targetPath, 'ios');
  } else {
    return Q();
  }
};
  ...
  try {
    if(options.isCordovaProject) {
      ...
      for (var x = 0; x < appSetup.plugins.length; x++) {
        Cordova.addPlugin(options.targetPath, appSetup.plugins[x], true);
      }

      // platform add android with --android flag
      if (options.android) {
        Cordova.addPlatform(options.targetPath, 'android', true);
      }

      // platform add ios with --android flag
      if (options.ios) {
        Cordova.addPlatform(options.targetPath, 'ios', true);
      }
