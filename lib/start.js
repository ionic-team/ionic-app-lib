...
Start.startApp = function startApp(targetPath, template, packageName, appName, isCordovaProject, setupSass) {
  // var self = this;
  // var q = Q.defer();
  // console.log('startApp template:', template)

  return Start.fetchWrapper(targetPath, isCordovaProject)
  .then(function(data){
    return Start.fetchSeed(targetPath, template);
  })
  .then(function(){
    return Start.loadAppSetup(targetPath);
  })
  .then(function(){
    return Start.initCordova();
  })
  .then(function(){
    return Start.setupSass();
  })
  .then(function(){
    return Start.finalize(targetPath);
  })
  .catch(function(err) {
    events.emit('log', 'Error: Unable to initalize app:', err.stack);
...
Start.fetchWrapper = function fetchWrapper(targetPath, isCordovaProject) {
  var q = Q.defer();
  // var self = this;

  var repoUrl = 'https://github.com/driftyco/' + IonicTask.WRAPPER_REPO_NAME + '/archive/master.zip';

  Ionic.fetchArchive(targetPath, repoUrl)
  .then(function() {
    var repoFolderName = IonicTask.WRAPPER_REPO_NAME + '-master';
    cp('-R', targetPath + '/' + repoFolderName + '/.', targetPath);
    ...
    if(!isCordovaProject) {
      ...
      for(var x=0; x<cordovaFiles.length; x++) {
        rm('-rf', targetPath + '/' + cordovaFiles[x]);
      }
    }
...
Start.fetchSeed = function(targetPath, template) {
  // console.log('fetchSeed is running targetPath:', targetPath, ' template: ', template)
  // Codepen: http://codepen.io/ionic/pen/GpCst
  if( /\/\/codepen.io\//i.test(template) ) {
    seedType = 'codepen';
    return Start.fetchCodepen(targetPath, template);
  }

  if( /creator:/i.test(template) ) {
    seedType = 'creator';
    return fetchCreatorApp();
  }

  // Github URL: http://github.com/myrepo/
  if( /\/\/github.com\//i.test(template) ) {
    seedType = 'github';
    return fetchGithubStarter(template);
  }

  // Local Directory: /User/starterapp
  if( (template.indexOf('/') > -1 || template.indexOf('\\') > -1) && (template.indexOf('http://') === -1 && template.indexOf('https://') === -1)) {
    // console.log('local', template);
    // TODO: seedType - how to pass back?
    seedType = 'local';
    return fetchLocalStarter();
  }

  // Ionic Github Repo
  seedType = 'ionic-starter';
  return Start.fetchIonicStarter(template);
};


Start.loadAppSetup = function loadAppSetup(targetPath) {
  var q = Q.defer(),
      appSetup = IonicTask.DEFAULT_APP;

  var appJsonPath = path.join(targetPath, 'www', 'app.json');
...
Start.fetchCodepen = function(targetPath, template) {
  // var self = this;
  var codepenUrl = template.split('?')[0].split('#')[0];
  var wwwPath = path.join(targetPath, 'www');
...
Start.fetchIonicStarter = function(template) {

  // Get the starter project repo name:
  var repoName = ['ionic-starter-', template].join('')

  // Get the URL for the starter project repo:
  var repoUrl = ['https://github.com/driftyco/', repoName].join('')

  return Start.fetchGithubStarter(repoUrl);
};


Start.fetchGithubStarter = function(repoUrl) {
  var self = this;
  var q = Q.defer();
...
Start.initCordova = function(isCordovaProject, appSetup, ios) {
  // var self = this;
  var q = Q.defer();

  if(isCordovaProject) {
    // update the config.xml file from cmd line args
    Start.updateConfigXml();
...
Start.updateConfigXml = function(targetPath, packageName, appName, appDirectory) {
  // var self = this;
  events.emit('log', 'Update config.xml');
  ...
  try {
    ...
    parseString(configString, function (err, jsonConfig) {
      ...
      if(!packageName) {
        packageName = appDirectory + (appDirectory !== 'tmp' ? Math.round((Math.random() * 899999) + 100000) : '');
...
Start.setupSass = function(hasSass) {
  if(hasSass) {
...
Start.finalize = function(targetPath) {
  // var self = this;

  Start.updateLibFiles(targetPath);
