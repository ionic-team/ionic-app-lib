...
Start.startApp = function startApp(options) {
  ...
  // console.log('startApp template:', options.template)
  ...
  .then(function(data){
    // console.log('FetchWrapper done')
    return Start.fetchSeed(options);
  })
  .then(function(){
    // console.log('finished fetchSeed')
    return Start.loadAppSetup(options);
  })
  .then(function(appSetup){
    // console.log('finished loadAppSetup - result: ', appSetup)
    options.appSetup = appSetup;
    return Start.initCordova(options);
  })
  .then(function(){
...
Start.initCordova = function(options) {
  // var self = this;
...
// Start.updateConfigXml = function(targetPath, packageName, appName, appDirectory) {
    ...
    Start.updateConfigXml(options.targetPath, packageName, appName);
