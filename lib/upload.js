...
function doUpload(appDirectory, jar, note, deploy) {
  ...
  var upload;
  try {
    return addCacheBusters(indexPath)
    .then(function() {
      return zipContents(appDirectory, documentRoot);
    })
    .then(function() {
      return removeCacheBusters(indexPath);
    })
    .then(function() {
      return getDirectUploadKey(project, jar, note);
    })
    .then(function(key) {
      ...
      project.save();
      return uploadToS3(appDirectory, key);
    })
    .then(function() { // receives status
      return signalDashUpload(project, jar);
    })
    .then(function(status) {
      ...
      return verify_tag(project, jar, deploy, version);
    })
    .then(function(deploy) {
      return deploy(project, jar, deploy);
    })
    .then(function() { // receives status
      return upload;
    })
...
function uploadToS3(appDirectory, keyInfo) {
  var q = Q.defer();
...
function signalDashUpload(project, jar) {
  var q = Q.defer();
...
function verify_tag(project, jar, deploy, version) { // eslint-disable-line camelcase
  var q = Q.defer();
...
function deploy(project, jar, deploy) {
  var q = Q.defer();
...
function getDirectUploadKey(project, jar, note) {
  var q = Q.defer();
...
function zipContents(appDirectory, documentRoot) {
...
module.exports = {
  ...
  signalDashUpload: signalDashUpload,
  verify_tag: verify_tag, // eslint-disable-line camelcase
  deploy: deploy,
  getDirectUploadKey: getDirectUploadKey,
  zipContents: zipContents,
  addCacheBusters: addCacheBusters,
  removeCacheBusters: removeCacheBusters
};
