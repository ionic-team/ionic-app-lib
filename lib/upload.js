...
Upload.doUpload = function doUpload(appDirectory, jar, note, deploy) {
  l.logger.info('Uploading app....'.green.bold);
  l.logger.debug('Upload doUpload - ', appDirectory, jar);

  if (deploy && deploy === true) {
    l.logger.info('Deploy channel not provided; defaulting to dev.');
  ...
  try {
    ...
    .then(function(key){
      ...
      .then(function(key){
        ...
        l.logger.error('Error saving app ID:', error);
      });
      ...
      if (status && (typeof status === 'object') && status.version) {
        ...
        if (status.api_key) {
          ioLib.writeIoConfig('api_key', status.api_key, true).then(function(apiKey){
            ...
            l.logger.error('Error saving API key:', error);
          });
        }
      }
    ...
    .catch(function(ex) {
      l.logger.error('An error occurred uploading the build: %s', ex, {});
      throw ex;
    });
  } catch(ex) {
    l.logger.error('Upload errors occurred - %s', ex, {});
  }
};
...
Upload.signalDashUpload = function signalDashUpload(project, jar) {
  ...
  l.logger.debug('Signaling to ionic.io completion of the upload');
  ...
  function(error, response, body) {
    if (error || response.statusCode != 200) {
      ...
      l.logger.info(('Successfully uploaded (' + project.get('app_id') + ')\n').bold);
      l.logger.info(('Share your beautiful app with someone:\n\n$ ionic share EMAIL\n').bold);
...
Upload.verify_tag = function verify_tag(project, jar, deploy, version) {
  ...
  l.logger.debug('Checking if we need to deploy the upload');
  ...
  if (deploy && version) {
    ...
    function(error, response, body) {
      if (error || response.statusCode != 200) {
        ...
        l.logger.debug('Verified channel tag...');
...
Upload.deploy = function deploy(project, jar, deploy) {
  ...
  l.logger.debug('Check for a deploy...');

  if (deploy) {
    l.logger.info('Deploying to channel: ' + deploy.channel.label.green.bold);
    ...
    function(error, response, body) {
      if (error || response.statusCode != 200) {
        ...
        l.logger.info('Deploy Successful!'.green.bold)
        q.resolve(true);
      }
    });
  } else {
...
Upload.getDirectUploadKey = function getDirectUploadKey(project, jar, note) {
  ...
  l.logger.debug('Getting Upload information from ', settings.IONIC_DASH);
  ...
  form.submit({
    ...
    if (err) {
      l.logger.info('There was an error trying to upload your app.'.red.bold);
      var errorMessage;
...
Upload.zipContents = function zipContents(appDirectory, documentRoot) {
  ...
  l.logger.debug('Now zipping contents of ' + path.join(appDirectory, documentRoot));
...
Upload.addCacheBusters = function addCacheBusters(indexPath) {
  ...
  l.logger.debug('When your webview is acting crazy who do you call? Cachebusters!');
...
Upload.removeCacheBusters = function removeCacheBusters(indexPath) {
  ...
  l.logger.debug('Removing cachebusting ', indexPath);
