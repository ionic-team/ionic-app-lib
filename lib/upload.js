...
Upload.doUpload = function doUpload(appDirectory, jar, note) {
  ...
  try {
    ...
    console.log('Upload errors occurred - ', ex);
  }
};
  ...
  form.submit({
    ...
    response.on('end', function() {
      if ( response.statusCode == 401 ) {
        ...
        return q.reject('Unknown error occurred with server');
      }
...
Upload.uploadZipContents = function uploadZipContents(projectd, jar, note) {
  ...
  var url = settings.IONIC_DASH + settings.IONIC_API + 'app/direct-upload/' + project.get('app_id');
