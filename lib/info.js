...
Info.printInfo = function printInfo(info) {
  ...
  if (info.ionic) {
    logging.logger.info('Ionic Version:', info.ionic);
  }
...
Info.checkRuntime = function checkRuntime() {
  ...
  if (nodeUpgrade || (!cordovaInstalled || cordovaUpgrade) ) {
    logging.logger.info('******************************************************'.red.bold);
    logging.logger.info(' Upgrade warning - for the CLI to run correctly,      '.red.bold);
    logging.logger.info(' it is highly suggested to install/upgrade the following:     '.red.bold);
    logging.logger.info('');
