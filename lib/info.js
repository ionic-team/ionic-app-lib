...
function getIonicCliVersion(info, ionicCliPath) {
  try {
...
function getIonicLibVersion(info) {
  try {
...
function getOsEnvironment(info) {
  switch (os.type()) {
  case 'Darwin':
    info.os = getMacInfo();
    info.xcode = getXcodeInfo();
    info.ios_sim = getIosSimInfo(); // eslint-disable-line camelcase
    info.ios_deploy = getIosDeployInfo(); // eslint-disable-line camelcase
    break;
  case 'Windows_NT':
    info.os = getWindowsEnvironmentInfo();
    break;
  case 'Linux':
    info.os = getLinuxEnvironmentInfo();
    break;
  }
}

function getNodeVersion(info) {
  info.node = process.version;
}

function gatherGulpInfo(info) {
...
function gatherInfo() {
  ...
  getIonicLibVersion(info);

  getNodeVersion(info);

  getOsEnvironment(info);
...
module.exports = {
  ...
  getCordovaInfo: getCordovaInfo,
  getXcodeInfo: getXcodeInfo,
  getIosSimInfo: getIosSimInfo,
  getIosDeployInfo: getIosDeployInfo,
  getIonicCliVersion: getIonicCliVersion,
  getIonicLibVersion: getIonicLibVersion,
  getIonicVersion: getIonicVersion,
  getWindowsEnvironmentInfo: getWindowsEnvironmentInfo,
  getLinuxEnvironmentInfo: getLinuxEnvironmentInfo,
  getOsEnvironment: getOsEnvironment,
  getNodeVersion: getNodeVersion,
  gatherGulpInfo: gatherGulpInfo,
