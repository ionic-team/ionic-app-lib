...
Info.getMacInfo = function getMacInfo() {
  ...
  // l.logger.info('Mac OS X', macVersion);
};
...
Info.printInfo = function printInfo(info) {
  ...
  l.logger.info('Cordova CLI:', info.cordova);
  if (info.gulp) {
    l.logger.info('Gulp version:', info.gulp);
    l.logger.info('Gulp local: ', info.gulp_local);
  }

  if (info.ionic) {
    l.logger.info('Ionic Version:', info.ionic);
  }

  if (info.ionic_cli) {
    l.logger.info('Ionic CLI Version:', info.ionic_cli);
  }

  if (info.ionic_lib) {
    l.logger.info('Ionic App Lib Version:', info.ionic_lib);
  }

  if (info.ios_deploy) {
    l.logger.info('ios-deploy version:', info.ios_deploy)
  }

  if(info.ios_sim) {
    l.logger.info('ios-sim version:', info.ios_sim);
  }

  l.logger.info('OS:', info.os);
  l.logger.info('Node Version:', info.node);

  if(info.xcode) {
    l.logger.info('Xcode version:', info.xcode);
  }
...
Info.checkRuntime = function checkRuntime() {
  ...
  if (nodeUpgrade || cordovaUpgrade ) {
    l.logger.info('******************************************************'.red.bold);
    l.logger.info(' Upgrade warning - for the CLI to run correctly,      '.red.bold);
    l.logger.info(' it is highly suggested to upgrade the following:     '.red.bold);
    l.logger.info('');

    if(nodeUpgrade) {
      var updateMessage = [' Please update your Node runtime to version ', requirements.node].join(' ');
      l.logger.info(updateMessage.red.bold);
      validRuntime = false;
    }
